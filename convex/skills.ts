...
- The final response names what changed and what still needs production/user verification.`,
  ...
  },
  {
    ...
    content: [
      ...
      "",
      "   ```bash",
      "   youmd machine prompt --root ~/Desktop/CODE_YOU --days 30",
      "   ```",
      "",
      ...
      "- Default to a first pass of projects active in the last 30 days, then ask before expanding to all active projects from the last 90 days.",
      ...
      "",
      "```bash",
      "youmd machine prompt --root /tmp/youmd-clean-host-CODE_YOU --days 30 --limit 80 --max-clone-projects 2",
      "```",
      "",
      "Generate a strict proof prompt that fails if the encrypted env vault is not",
      "present/restored:",
      "",
      "```bash",
      ...
      "```",
      "",
      "Large restored roots can make local evidence hydration expensive. The",
      "generated command bounds portfolio hydration by default; override when",
      "needed:",
      "",
      "```bash",
      "YOUMD_PORTFOLIO_HYDRATE_TIMEOUT_SECONDS=300 youmd machine prompt --root ~/Desktop/CODE_YOU --days 30 --limit 80 --require-env-vault",
      "```",
      "",
      ...
      "  printing raw values.",
      "- If real `.env.local` restore is required for the setup proof, the generated",
      "  command used `--require-env-vault` / `YOUMD_REQUIRE_ENV_VAULT=1`; do not",
      "  mark the fresh-machine setup done if app repos still report `needs-env`.",
      "- Portfolio hydration used the generated timeout guard, or the operator",
      "  intentionally increased `YOUMD_PORTFOLIO_HYDRATE_TIMEOUT_SECONDS`.",
      "- The 30-day first pass is complete, and the 90-day expansion is either explicitly accepted and verified or intentionally left open.",
