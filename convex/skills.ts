...
- The final response names what changed and what still needs production/user verification.`,
  ...
  },
  {
    ...
    content: [
      ...
      "   ```",
      "",
      "9. After projects are cloned and any encrypted env vault is restored, reconcile",
      "   You.md state again, then hydrate the portfolio graph so source-Mac bundle",
      "   updates, new trusted-device envelopes, local README, project-context, git,",
      "   PR, and env-key evidence become visible to future agents:",
      "",
      "   ```bash",
      "   youmd pull",
      "   youmd sync",
      ...
      "- Publish machine setup milestones through the realtime agent bus so Houston's",
      "  source Mac and other trusted devices can see progress without babysitting the",
      "  clipboard:",
      "  `youmd agent send --channel machine-sync --kind status \"fresh machine $(hostname) finished <phase>\"`.",
      ...
      "  source-Mac Secret Vault share if no trusted-device envelope exists yet,",
      "  local vault passphrase/Keychain only when",
      "  `YOUMD_ALLOW_LOCAL_ENV_VAULT_FALLBACK=1` or `YOUMD_ENV_VAULT` is explicitly",
      "  provided, npm OTP, OS permissions, or the explicit 90-day project expansion choice.",
      ...
      "Fresh-machine setup should treat this as the default gate, not as a reason to",
      "fall through to a passphrase prompt on the new Mac.",
      "",
      "Fallback path is explicit opt-in only: create an encrypted archive from the old",
      "machine in an interactive macOS terminal and transfer the encrypted file by",
      "iCloud, AirDrop, USB, or private password-manager attachment:",
      "",
      "```bash",
      ...
      "```",
      "",
      "Only when the command was explicitly run with",
      "`YOUMD_ALLOW_LOCAL_ENV_VAULT_FALLBACK=1` or `YOUMD_ENV_VAULT=/path/to/vault`",
      "should a headless agent use the macOS Keychain fallback for the env-vault",
      "passphrase. Input is silent and the value is not printed:",
      "",
      "```bash",
