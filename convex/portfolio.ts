...
export const upsertMachineProof = mutation({
  args: {
    ...
    daemonsLoaded: v.optional(v.number()),
    daemonsTotal: v.optional(v.number()),
    legacyDaemonsLoaded: v.optional(v.number()),
    daemonWarnings: v.optional(v.array(v.string())),
    daemonLabels: v.optional(v.array(v.string())),
    failures: v.number(),
  ...
  handler: async (ctx, args): Promise<{ proofId: Id<"machineProofReports">; created: boolean }> => {
    ...
    const row = {
      ...
      daemonsLoaded: Math.max(0, Math.trunc(args.daemonsLoaded ?? 0)),
      daemonsTotal: Math.max(0, Math.trunc(args.daemonsTotal ?? 0)),
      legacyDaemonsLoaded: Math.max(0, Math.trunc(args.legacyDaemonsLoaded ?? 0)),
      daemonWarnings: optionalStrings(args.daemonWarnings).slice(0, 12),
      daemonLabels: optionalStrings(args.daemonLabels).slice(0, 12),
    ...
    if (existing) {
      ...
      await recordBrainActivity(ctx, {
        ...
        detail: `${row.ready}/${row.scanned} projects ready · ${row.daemonsLoaded}/${row.daemonsTotal} daemons · ${row.needsEnv} need env · ${row.failures} failures`,
        status: row.status,
        ...
        metadata: {
          ...
          daemonsLoaded: row.daemonsLoaded,
          daemonsTotal: row.daemonsTotal,
          legacyDaemonsLoaded: row.legacyDaemonsLoaded,
          daemonWarningCount: row.daemonWarnings.length,
