...
- [ ] Publish `youmd@0.6.19` so end users get the `you` launcher, compact portrait splash, paced startup investigation, public-profile portrait contract fix, update hints, read-only active-bundle fallback, raw-markdown identity roundtrip preservation, marker-based recent-project opportunity scanning, broader local workspace repo awareness, local host-tool execution for `start there` and generic recent-work scans, the stronger onboarding handoff into U, the narrow-terminal portrait encounter stacking fix, the deeper home-level context sweep, and a much more concise strongest-move opener
