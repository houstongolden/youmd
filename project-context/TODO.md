...
- [x] Add a local host-tool bridge for CODE_2025 recent-work scanning so U can answer “what have I been working on lately?” from real filesystem state with a remote-model final response
- [ ] Publish `youmd@0.6.16` so end users get the `you` launcher, compact portrait splash, paced startup investigation, public-profile portrait contract fix, update hints, read-only active-bundle fallback, raw-markdown identity roundtrip preservation, recent-project opportunity scanning from arbitrary directories, broader local workspace repo awareness, local host-tool execution for `start there` and CODE_2025 scans, the stronger onboarding handoff into U, the narrow-terminal portrait encounter stacking fix, the deeper home-level context sweep, and a much more concise strongest-move opener
