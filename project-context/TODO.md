...
- [ ] Run the generated command end-to-end on the actual brand-new computer / clean agent host with a real encrypted env vault and verify projects, skills, env vault restore, local servers, and portfolio graph sync there
...
- [ ] After this commit is pushed and Convex deploys from the new `main`, rerun authenticated browser proof that changing a project focus status persists and the dropdown recovers from backend deploy gaps
