...
- [ ] Publish `youmd@0.6.12` so end users get the `you` launcher, portrait splash, truthful startup investigation, public-profile portrait contract fix, update hints, read-only active-bundle fallback, raw-markdown identity roundtrip preservation, recent-project opportunity scanning from arbitrary directories, broader local workspace repo awareness, recent-project openings that prioritize actionable workspace repos like `foldermd`, the stronger onboarding handoff into U, and the narrow-terminal portrait encounter stacking fix
