...
- [x] Add local action handling for `start there` so U can act on its own strongest project suggestion instead of immediately asking for a URL/repo
- [x] Add local CODE_2025 recent-work scanning so U can answer “what have I been working on lately?” from real filesystem state
- [ ] Publish `youmd@0.6.15` so end users get the `you` launcher, compact portrait splash, paced startup investigation, public-profile portrait contract fix, update hints, read-only active-bundle fallback, raw-markdown identity roundtrip preservation, recent-project opportunity scanning from arbitrary directories, broader local workspace repo awareness, local `start there` execution, local CODE_2025 recent-work scanning, the stronger onboarding handoff into U, the narrow-terminal portrait encounter stacking fix, the deeper home-level context sweep, and a much more concise strongest-move opener
