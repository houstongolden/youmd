...
- [x] Add a local host-tool bridge for generic recent-work scanning so U can answer “what have I been working on lately?” from real filesystem state with a remote-model final response
- [ ] Publish `youmd@0.6.18` so end users get the `you` launcher, compact portrait splash, paced startup investigation, public-profile portrait contract fix, update hints, read-only active-bundle fallback, raw-markdown identity roundtrip preservation, recent-project opportunity scanning from arbitrary directories, broader local workspace repo awareness, local host-tool execution for `start there` and generic recent-work scans, the stronger onboarding handoff into U, the narrow-terminal portrait encounter stacking fix, the deeper home-level context sweep, and a much more concise strongest-move opener
