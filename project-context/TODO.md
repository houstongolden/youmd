...
- [ ] Create the real encrypted env vault at `~/Desktop/youmd-env-vault/env-vault-*.tar.enc` from the source Mac. Preflight sees 17 `.env.local` files plus 3 agent-auth files, but no vault exists yet; because no GPG recipient/1Password/age recipient is configured, this must be created through the interactive passphrase path rather than faking a non-interactive backup
...
- [x] Convert Portfolio project details from an inline lower-page appendix into a dedicated URL-backed drill-in state: `/shell?tab=portfolio` stays a compact project list, while `/shell?tab=portfolio&project=bamfaiapp#project-detail` hides global portfolio sections and shows project overview, strategy, timeline anchors, and `<< back to projects`
- [ ] Apply the same compact list -> dedicated detail page pattern to Stacks, APIs/env provider/surface rows, Skills propagation, task triage, and brain-dump records so shell tabs do not become long mixed overview/detail pages
