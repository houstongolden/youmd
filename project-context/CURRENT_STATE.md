...
- Production fix deployed in `c80ccb8`: manual sidebar expansion now overrides the auto-collapse breakpoint, so the left sidebar can reopen on wide production layouts after it was collapsed. Follow-up deployment `dpl_9UrW7btDtfSAHMd9jsdHwPmBCY73` (`94a2d56`) is Ready and aliased to `https://www.you.md`; authenticated Chrome QA on `https://www.you.md/shell` verified the production shell loads in Houston's session, the sidebar expands with Home/Projects/Skills labels visible, and real session-derived machine counts render as `5` in the title bar, sidebar, Home sync strip, Home brief, and Machines card.
...
- `youmd sync --live` includes agent-bus messages in the sync head and materializes `~/.youmd/realtime-sync-status.json` plus `~/.youmd/agent-bus/inbox.json` with `secretValuesExposed: false`.
- Fresh-machine prompts and hosted `machine-bootstrap` now instruct Claude/Codex to send Mac mini setup milestones through `youmd agent send --channel machine-sync --kind status ...`.
- Source-Mac proof: live `agent send` + websocket materialization shows `2` recent agent messages, latest `2026-06-18T09:43:37.647Z`, with zero raw `ym_` / `sk-` / env-key leaks in local status JSON.
- Remaining tonight: publish `youmd@0.8.6`, rerun the Mac mini prompt, and confirm a reply from the Mac mini appears on this source Mac.
...
- Shell detail drill-ins now use dedicated hard-refreshable paths instead of query/hash pseudo-pages: Portfolio projects open at `/shell/projects/<slug>` and `/shell/projects/<slug>#timeline`, YouStacks open at `/shell/stacks/<slug>`, and Skills open at `/shell/skills/<name>`. Authenticated Codex in-app Browser QA verified `/shell/projects/bamfaiapp`, list click-through, `<< back to projects` returning to `/shell?tab=portfolio`, direct timeline hash scrolling, stack detail/back, and skill detail/back; screenshot `/tmp/youmd-shell-project-dedicated-route-proof-2026-06-17.png`.
...
- Trusted-device realtime sync now exists as a first concrete Notion-like sync lane. `POST /api/v1/me/realtime-sync/session` mints short-lived `ys_` websocket credentials from authenticated API keys; `realtimeSync.getHead` returns secret-safe identity/skill/portfolio/repo/machine heads plus account-backed Secret Vault snapshot readiness (`ready` / `missing` / `scope-missing`) when the key has `vault` scope. CLI `youmd sync --live --daemon` subscribes to Convex, pulls identity, re-renders installed skills, triggers bounded shared stack/project-context sync, and writes `~/.youmd/realtime-sync-status.json` with safe vault metadata and pull/restore commands. `com.youmd.realtime-sync` is installed locally with `KeepAlive=true` and appears as `realtime brain / live websocket` in `youmd stack daemon status` plus the Machine pane. Source-Mac account Secret Vault snapshot is now uploaded and realtime-ready: `16` projects / `451` variables from `Houstons-MBP.lan`, with no manifest text, archive bytes, or secret values returned by list/realtime status. Latest CLI target is `0.8.6`; npm publish is required before new machines get this path from npm/npx.
- Follow-up strict real-vault disposable-host proof is now complete locally and synced server-side. The uncapped graph-backed setup path selected `43` active projects, cloned `41` git repos and created `2` directory-only project folders under `/tmp/strict-real-env-bootstrap-20260617T185635Z/CODE_YOU`, then a strict generated-command rerun restored a real encrypted vault into that root from a temp home. Latest owner-gated proof row: `48` scanned dirs, `41` git repos, `33` package projects, `17` `.env.local`, `17` env examples, `25` ready, `8` needs env, `4` partial, dependency installs `2 passed / 1 failed`, server probes `1 passed / 1 failed`, and `secretValuesExposed: false`. Two real bootstrap blockers were found and addressed/recorded: `machine projects --yes` could spin after clone completion and is now forced to exit cleanly, while large-root portfolio hydration is now bounded by `YOUMD_PORTFOLIO_HYDRATE_TIMEOUT_SECONDS` in generated CLI/web-shell prompts. Actual separate new-computer proof and authenticated visual Machine-pane proof remain open.
...
### CLI (youmd v0.8.6 — local build / npm publish pending)
