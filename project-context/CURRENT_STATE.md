...
- The Portfolio Graph project browser now has a compact operating view with search, focus filter, sort, density toggle, URL-backed project detail selection, shipped `today` / `7d` / `30d` / `90d` counters, and selected-project detail panels that show project links, API/MCP/stack surface docs URLs, curl commands, associated stack, focus/goal, and dependency snapshot. Project focus status/rank is now persisted in Convex via owner-gated `portfolio.updateProjectFocus`; the UI has a timeout guard so a pre-deploy backend mismatch cannot leave the focus dropdown permanently disabled. Post-deploy authenticated in-app Browser proof changed `youmd` from `focusing` back to `top-priority` and read back the persisted enabled state at `/shell?project=youmd`.
