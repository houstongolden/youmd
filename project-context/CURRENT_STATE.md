...
Last Commit: see git log for latest 2026-04-18 `you` launcher + portrait splash continuation
...
### CLI (youmd v0.6.4 — ready to publish)
...
- npm publish retry path is fixed: the next release target is `0.6.4`, package metadata is normalized, and the built CLI + MCP user-agent now match that version cleanly
...
- The published npm package on npm is still `0.6.2`; the latest CLI fixes in this repo are `0.6.4` and need one more npm publish before end users get the `you` launcher, portrait splash, update hints, and raw-markdown roundtrip preservation
- The new `you` experience works well locally, but some non-chat commands such as `status` still behave local-bundle-first instead of using the same home-bundle fallback semantics
