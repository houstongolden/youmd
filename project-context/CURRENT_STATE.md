...
### CLI (youmd v0.6.13 — ready to publish)
...
- npm publish retry path is fixed: the next release target is `0.6.13`, package metadata is normalized, and the built CLI + MCP user-agent now match that version cleanly
...
- The published npm package on npm is still behind the repo; the latest CLI fixes in this repo are now `0.6.13` and need one more npm publish before end users get the `you` launcher, portrait splash, truthful startup investigation, update hints, public-profile portrait contract fix, active-bundle fallback on read-only commands, raw-markdown roundtrip preservation, recent-project opportunity scanning from arbitrary directories, broader local workspace repo awareness, actionable recent-project openings, a stronger onboarding handoff into U, the narrow-terminal portrait encounter fix, and the deeper home-context sweep + strongest-move opener
