...
### CLI (youmd v0.6.6 — ready to publish)
...
- The published npm package on npm is still behind the repo; the latest CLI fixes in this repo are now `0.6.6` and need one more npm publish before end users get the `you` launcher, portrait splash, truthful startup investigation, update hints, public-profile portrait contract fix, active-bundle fallback on read-only commands, and raw-markdown roundtrip preservation
