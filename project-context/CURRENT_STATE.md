...
- Trusted-device realtime sync now exists as a first concrete Notion-like sync lane. `POST /api/v1/me/realtime-sync/session` mints short-lived `ys_` websocket credentials from authenticated API keys; `realtimeSync.getHead` returns secret-safe identity/skill/portfolio/repo/machine/vault-metadata heads; CLI `youmd sync --live --daemon` subscribes to Convex, pulls identity, re-renders installed skills, and triggers bounded shared stack/project-context sync. `com.youmd.realtime-sync` is installed locally with `KeepAlive=true` and appears as `realtime brain / live websocket` in `youmd stack daemon status` plus the Machine pane. Production smoke verified `56` projects, `10` skills, `14` tasks, and `secretValuesExposed: false`; CLI is bumped to `0.8.4` and needs npm publish for new machines.
...
### CLI (youmd v0.8.4 — local build / npm publish pending)
