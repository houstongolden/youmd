...
### CLI (youmd v0.6.5 — ready to publish)
...
- npm publish retry path is fixed: the next release target is `0.6.5`, package metadata is normalized, and the built CLI + MCP user-agent now match that version cleanly
...
- The published npm package on npm is still `0.6.2`; the latest CLI fixes in this repo are `0.6.5` and need one more npm publish before end users get the `you` launcher, portrait splash, update hints, active-bundle fallback on read-only commands, and raw-markdown roundtrip preservation
