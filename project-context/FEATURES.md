...
| Loop Reports | Foundation UI + backend code complete / first DSI adapter wired | Added `loopReportDefinitions`, `sourceSnapshots`, `loopReportRuns`, and `loopReportArtifacts`, owner CRUD/listing/manual-run functions, an hourly due-loop cron, deterministic daily briefing compilation from You.md-owned activity/projects/sources/repo mirror/memories/pending source changes/DSI components, focused Convex tests, and owner-visible `/shell` controls to seed defaults, run the daily briefing, pause/resume definitions, inspect runs, open generated artifacts, and drill into normalized source snapshots with hashes/trust/capture windows. External adapters remain pending for Perplexity trends, calendar/tasks, BAMF/BAMF OS, Bad.app, school crawler, and richer daily journal generation. Detailed plan: `ARTIFACTS_DAILY_LOOPS_DSI_PUBLIC_CHAT_PLAN_2026-06-16.md` |
| DSI catalog components | First private weather/surf/GitHub project slices code complete | Added `dsiComponents` as the first extensible structured component table for the personal API/MCP layer, plus owner-gated refresh actions for h.computer-style Open-Meteo weather, Venice Breakwater surf calibration, and a GitHub project catalog from tracked projects + repo mirror stats. `/shell` Reports now shows a DSI catalog panel with counts, trust/visibility metadata, refresh controls, summaries, and read-only markdown artifacts under `dsi/private/*`. GitHub catalog includes project count, GitHub URLs, matched clean project URL, recent commits, stars, AI insight, privacy/visibility, and exact LOC/LOMB/LOMB ratio for the mirrored repo; full per-repo GitHub `/languages` LOC/LOMB enrichment remains next. Remaining: owner-approved public profile rendering, custom component schema/editor, BAMF social analytics, Bad.app fitness, school logistics, and research/blog source trackers |
| Public profile chat | API + MCP first slice code complete / owner controls pending | Public profiles now expose an above-the-fold chat widget backed by `POST /api/v1/profiles/[username]/conversation`, and hosted MCP exposes `ask_public_profile` with the same public-context-only contract. The surfaces answer only from the public profile payload, return source links/public fields used/suggested follow-ups, and explicitly omit private memories, reports, connected-app data, source snapshots, logs, and scoped API/MCP grants. Remaining: LLM voice tuning, owner controls for style/capabilities/public fields, local MCP parity if needed, and production verification |
