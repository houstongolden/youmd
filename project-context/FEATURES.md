...
| Agent-readable docs surfaces | Done deployed / generation upgrade pending deploy | `/llms.txt` exposes the short agent index and `/llms-full.txt` exposes the full plain-text agent context pack; `/docs#agent-docs`, `robots.txt`, and `sitemap.xml` point agents to docs, API, MCP, runtime, YouStacks, smoke checks, and upstream reference intelligence. The root files now have a generated pipeline (`llms:generate` / `llms:check`) that pulls docs-reference counts, source hash, CLI version, selected endpoint/tool summaries, and upstream reference heads; production deployment `dpl_GcSaYeSrzo1JRaqVa9MAyMr4J2VY` is live for the first static version, generation upgrade deploy pending |
...
| Codex chat hygiene | Done local | You.md now has a documented policy and transcript index for consolidating recurring automation chats; the duplicate daily reference automation is paused, completed automation threads are archived from the active Codex sidebar, and durable outputs live in project-context files plus automation memory |
