...
| Agent-readable docs surfaces | Done deployed | `/llms.txt` exposes the short agent index and `/llms-full.txt` exposes the full plain-text agent context pack; `/docs#agent-docs`, `robots.txt`, and `sitemap.xml` now point agents to docs, API, MCP, runtime, YouStacks, smoke checks, and upstream reference intelligence; production deployment `dpl_GcSaYeSrzo1JRaqVa9MAyMr4J2VY` is live |
