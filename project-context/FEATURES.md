...
| Agent docs CI guardrail | Done local / deploy pending | `npm run agent-docs:ci` runs generated docs checks, generator/smoke script syntax checks, and targeted lint; `.github/workflows/agent-docs.yml` runs that check on path-scoped push, pull request, or manual triggers for agent-docs source surfaces |
