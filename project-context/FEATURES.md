...
| Machine bootstrap skill | Production graph-backed planning verified / real new-host run pending | Added bundled `machine-bootstrap`, `youmd machine projects`, and `youmd machine prompt` so a fresh computer can install/auth/pull/sync You.md, restore shared agent skills/stacks, create a Desktop code root such as `CODE_YOU`, hydrate the Portfolio Graph from You.md/GitHub before cloning, create/clone active project repo directories, optionally restore an encrypted env vault, rehydrate local README/project-context/env-key evidence, and start resident sync daemons. The signed-in shell handles `/new computer`, `/new machine`, and obvious natural-language setup requests by minting a 7-day scoped bootstrap key and returning a single copyable Claude Code/Codex command while keeping raw `.env.local` values out of the browser prompt. The project command now fetches secret-safe `GET /api/v1/me/portfolio/graph`, merges persisted portfolio graph projects, graph-tracked GitHub repos, live `gh` repo scans, and local bundle records, prints source counts, uses repo names as directory names, tags associated stacks, attaches API/MCP docs links, skips duplicates, asks before older projects in interactive mode, supports dry-run/directory-only modes, and clones with `gh repo clone` or `git clone`. The one-command prompt now previews the graph-backed setup plan before cloning. Production dry-run proof against deployed Convex showed `55` portfolio projects, `40` tracked repos, and `40` cloneable graph-backed repos selected after fixing false-positive badge/docs URL detection. Verified with focused CLI tests/build, root typecheck, docs generation/check, Convex codegen/deploy, production graph dry-run, compiled CLI prompt smoke, and authenticated browser QA of `/new computer`; screenshot: `/tmp/youmd-new-computer-prompt-2026-06-17.png`. Next: run the generated command on the actual new machine/clean host, add a post-clone local readiness/run checker, add dedicated machine/computer readiness confirmation, and reseed hosted `/api/v1/skills` |
