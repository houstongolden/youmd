...
| Lovable-simple connector UX | Planned | Dashboard flow should let users add sources/tools by choosing a connector or URL, previewing mapped context, setting visibility/trust rules, and choosing refresh policy. First targets: GitHub, URL/RSS, raw markdown/OKF, h.computer, folder.md, BAMF.ai, social sources, custom webhook, and custom JSON endpoint |
| Source refresh and monitored updates | Partially founded / planned UX | Immutable raw-source versioning and provenance work gives the backend substrate; remaining product layer is per-source refresh policies, source health, monitored change summaries, approval/writeback, and user-facing connector status |
