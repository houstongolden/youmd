...
| Clean npm publish metadata | Done | CLI package now targets `0.6.15`, uses normalized `bin` paths, and ships the canonical git+https repository URL so npm no longer needs to auto-correct those fields during publish |
...
| Local chat action layer | Done | `you` / `youmd chat` now intercept `start there` and CODE_2025 recent-work questions locally, so U acts from real filesystem context before involving the remote model |
