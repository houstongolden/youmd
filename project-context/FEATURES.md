...
| YouStacks homepage section | Done deployed | Homepage now leads with "package your expertise into your own GStack": years of expertise, skills, sub-agents, prompts, workflows, taste, examples, tool rules, and safe You.md memory access packaged for any agent; production deployment `dpl_EyuaBhd5yXGFrw5eAGu2eBqZ46su` is ready and aliased |
| YouStacks docs chapter | Done deployed | `/docs#youstacks` now explicitly uses the Gary Tan/GStack analogy, frames stacks as personal expertise/workflow systems, adds sub-agents to "What Goes In", and keeps use cases, CLI, install flow, manifest, examples, API/MCP threshold, endpoint reference, and stack MCP tools together; production deployment `dpl_EyuaBhd5yXGFrw5eAGu2eBqZ46su` is live |
