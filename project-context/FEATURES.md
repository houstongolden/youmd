...
| Shell composer attach + voice polish | Done local / deploy pending | The main chat composer no longer renders the orange `>` prompt glyph or inner orange input outline; focus now highlights only the outer composer shell. The bottom utility row has a standard `+` attach control for images and readable text/code files plus a `cmd/ctrl shift m` voice shortcut affordance using browser speech recognition where available, with a clean fallback path for future Whisper-backed transcription |
