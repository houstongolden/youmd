...
| Repo-native You.md (own GitHub repo as source of truth) | Phase 2 done / Phases 3–5 planned | Host the full You.md `.md` + stacks in the user's own public/private GitHub repo and clone/mirror server-side for API/MCP. Schema (`githubConnections.repo*`) supports the linked repo; create/connect/seed shipped. Remaining: sync engine (Phase 3), server mirror (Phase 4), GitHub App hardening (Phase 5). Phased design in `GITHUB_NATIVE_PLAN.md` |
