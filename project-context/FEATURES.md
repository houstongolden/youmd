...
|---|---|---|
| Personal API/MCP product boundary | Planned / documented | 2026-06-15/16 routing passes moved the productizable h.computer/creator-new ideas into You.md: You.md owns the personal API/MCP, structured identity/context contract, durable agent brain, connected source memory, scoped grants, provenance-rich writeback, YouStacks, host adapters, source refresh, skill learning, and stack-level model routing. h.computer remains Houston's personal site/reference implementation that reads from You.md and writes useful memories/activity back. Current preserved messaging includes "A personal API where the context is you," "The context every agent should already have," and "Brain -> Stacks -> Runtime -> Protected API/MCP." See `PERSONAL_API_MCP_AND_YOUSTACKS_MEMO_2026-06-15.md` and `prompts/2026-06-16-youmd-personal-api-context-routing.md` |
...
| Shell GitHub update chrome | MVP code complete / orchestration pending | Shell top chrome now right-aligns repo state with a solid GitHub mark, Convex repo-mirror status colors, hover-only repo name, compact last-sync label, and one `[ update ]` action. Current button streams an update preflight into chat and runs the existing publish path. Next: real PR/merge/conflict orchestration, expandable update artifacts, commit/PR message persistence, and history linkage |
...
| Artifact workspace | MVP code complete / report history foundation added | Files pane now has `files`, `artifacts`, and `reports` modes plus markdown report templates for daily briefing, project carryover, daily journal article, and public profile chat contract. Templates save through `custom_files`, keeping them portable through the existing bundle/API/MCP path. Generated private Loop Report artifacts now appear as read-only markdown under `reports/generated/*`, and the Reports workspace shows owner controls for definitions, runs, artifacts, and manual daily runs. Next: richer editing/versioning, provenance/backlinks, and DSI publish controls |
| Loop Reports | Foundation UI + backend code complete / adapters pending | Added `loopReportDefinitions`, `sourceSnapshots`, `loopReportRuns`, and `loopReportArtifacts`, owner CRUD/listing/manual-run functions, an hourly due-loop cron, deterministic daily briefing compilation from You.md-owned activity/projects/sources/repo mirror/memories/pending source changes, focused Convex tests, and owner-visible `/shell` controls to seed defaults, run the daily briefing, pause/resume definitions, inspect runs, and open generated artifacts. External adapters remain pending for Perplexity trends, calendar/tasks, BAMF/BAMF OS, Bad.app, weather, surf, school crawler, and richer daily journal generation. Detailed plan: `ARTIFACTS_DAILY_LOOPS_DSI_PUBLIC_CHAT_PLAN_2026-06-16.md` |
| DSI catalog components | Planned / plan captured | Owner-approved structured components for weather, surf, GitHub project catalog with LOC/LOMB/recent updates, agent activity, BAMF social analytics, Bad.app fitness, school logistics, and research/blog source trackers. These should render privately in `/shell` and optionally on public You.md profiles |
| Public profile chat | Planned / plan captured | Every public profile should expose a secure above-the-fold chat widget and `conversation` API/MCP surface that answers from public context in that user's voice, with owner controls and strict private-context isolation |
...
| Local agent browser auth handoff | Done local / CLI publish pending | `youmd login` now uses an Enter-to-open `/auth` browser approval flow, `/auth` shows a branded portrait-aware success confirmation for web + local agent auth, and the curl installer points fresh machines through login/pull/sync/`you`; CLI bumped/built as `0.8.2` pending npm OTP publish |
