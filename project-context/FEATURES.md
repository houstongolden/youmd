...
| Repo-native You.md (own GitHub repo as source of truth) | Phases 2–3(slice) done / 3-followups + 4–5 planned | Host the full You.md `.md` + stacks in the user's own public/private GitHub repo and clone/mirror server-side for API/MCP. Done: connect/create/seed + push/pull sync of you.md/you.json. Remaining: sync `private/*` + `stacks/*`, 3-way merge + webhook auto-pull (Phase 3), server mirror (Phase 4), GitHub App hardening (Phase 5). Phased design in `GITHUB_NATIVE_PLAN.md` |
