...
| YouStacks homepage section | Done local / deploy pending | Homepage now leads with "package your expertise into your own GStack": years of expertise, skills, sub-agents, prompts, workflows, taste, examples, tool rules, and safe You.md memory access packaged for any agent; previous deployment `dpl_G27RMotWbEVEFo3kjqPepbc7SzZo` remains live until this clarification deploys |
| YouStacks docs chapter | Done local / deploy pending | `/docs#youstacks` now explicitly uses the Gary Tan/GStack analogy, frames stacks as personal expertise/workflow systems, adds sub-agents to "What Goes In", and keeps use cases, CLI, install flow, manifest, examples, API/MCP threshold, endpoint reference, and stack MCP tools together |
