...
| Public profile chat | First slice code complete / owner controls pending | Public profiles now expose an above-the-fold chat widget backed by `POST /api/v1/profiles/[username]/conversation`. The endpoint answers only from the public profile payload, returns source links/public fields used/suggested follow-ups, and explicitly omits private memories, reports, connected-app data, source snapshots, logs, and scoped API/MCP grants. Remaining: LLM voice tuning, owner controls for style/capabilities/public fields, MCP tool parity, and production verification |
