...
| Resilient full-context links | Done | `/ctx` resolution now treats use-count/view/activity writes as best-effort, returns explicit JSON errors instead of platform 5xxs, and sends tokenized context responses with private/no-store cache headers plus `Vary: Accept`; production verified with full JSON, text/plain, invalid-token, and ETag checks |
