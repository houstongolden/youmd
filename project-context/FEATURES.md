...
| Agent docs CI guardrail | Done deployed | `npm run agent-docs:ci` runs generated docs checks, generator/smoke script syntax checks, README/root-agent-manual/docs-page/PRD/architecture handoff marker checks, forbidden stale stack/auth marker checks, and targeted lint; the handoff marker check derives the expected root agent-manual CLI version from `cli/package.json` plus app stack versions from `package.json`, so `AGENTS.md` and `CLAUDE.md` must stay aligned with the real package versions and first-party passwordless auth model. `/llms-full.txt`, `/docs#agent-docs`, and `npm run llms:smoke` now document and verify the expanded source-repo guardrail scope, with source-repo guardrail wording reported as its own live smoke check. `.github/workflows/agent-docs.yml` runs that check on path-scoped push, pull request, or manual triggers for agent-docs source surfaces including README, `AGENTS.md`, `CLAUDE.md`, `/docs#agent-docs`, and active PRD/architecture docs. GitHub Actions run `26853627596` passed and production deployment `dpl_6Y696yy1PZyoCQ1Efy38Y7MBvRbe` is live |
