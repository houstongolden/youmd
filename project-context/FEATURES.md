...
| Agent-readable docs surfaces | Done local / deploy pending | `/llms.txt` exposes the short agent index and `/llms-full.txt` exposes the full plain-text agent context pack; `/docs#agent-docs`, `robots.txt`, and `sitemap.xml` now point agents to docs, API, MCP, runtime, YouStacks, smoke checks, and upstream reference intelligence |
