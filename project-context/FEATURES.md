...
| Artifact workspace | MVP code complete / needs richer backend | Files pane now has `files`, `artifacts`, and `reports` modes plus markdown report templates for daily briefing, project carryover, daily journal article, and public profile chat contract. Templates save through `custom_files`, keeping them portable through the existing bundle/API/MCP path. Next: dedicated artifact/report tables, provenance/backlinks, source snapshots, and generated report history |
| Loop Reports | Planned / plan captured | You.md-native scheduled reports should gather Perplexity trends, calendar/tasks, GitHub/code carryover, connected-app analytics, Bad.app fitness, BAMF/BAMF OS data, school crawler data, weather, surf, research/blog crawlers, and agent activity, then write private markdown/JSON artifacts with citations and optional public DSI outputs. Detailed plan: `ARTIFACTS_DAILY_LOOPS_DSI_PUBLIC_CHAT_PLAN_2026-06-16.md` |
