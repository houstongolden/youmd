...
| Shell logo/sidebar toggle polish | Done deployed / visual approval pending | The separate Lucide left-sidebar toggle beside the YOU mark is removed. The PixelYOU mark is now the sidebar toggle itself, and hover subtly swaps the logo into a two-line menu glyph. The collapsed mark was further tightened to `scale(.094)` inside a 36px viewport so `YOU` fits cleanly in the skinny rail. The right-pane toggle moved out of the left side of the top chrome and into the far-right top chrome using a cleaner minimal side-panel glyph |
