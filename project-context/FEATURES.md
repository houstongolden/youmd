...
| Clean npm publish metadata | Done | CLI package now targets `0.6.16`, uses normalized `bin` paths, and ships the canonical git+https repository URL so npm no longer needs to auto-correct those fields during publish |
...
| Local host-tool bridge | Done | `you` / `youmd chat` now execute `start there` and CODE_2025 recent-work scans locally, then send the tool result through the remote model for the final response |
