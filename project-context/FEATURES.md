...
| Shell right-pane responsiveness | Done deployed / visual approval pending | Desktop shell split now gives the right detail pane a real minimum width, clamps stored over-wide chat ratios, and only uses split-screen mode on large desktops; tablet/narrow desktop uses the full-width shell/preview toggle. The profile detail pane now renders the compact actionable inspector instead of the full public profile page, and the pane navigation uses shorter wrapping labels so it does not clip into unusable horizontal overflow. Vercel deployment `dpl_Cwjg25ybGryr6ZYmCEjrtvEfLxGM` reached Ready; authenticated production QA confirmed no horizontal overflow, ~597px chat / ~751px detail at 1600px, and full-width preview mode at 900px |
