...
| Agent docs CI guardrail | Done deployed | `npm run agent-docs:ci` runs generated docs checks, generator/smoke script syntax checks, README/root-agent-manual handoff marker checks, and targeted lint; `.github/workflows/agent-docs.yml` runs that check on path-scoped push, pull request, or manual triggers for agent-docs source surfaces including README, `AGENTS.md`, and `CLAUDE.md`. GitHub Actions run `26850497617` passed and production deployment `dpl_DX4g1vvZShDE3BaAxprMVhMfpiiw` is live |
