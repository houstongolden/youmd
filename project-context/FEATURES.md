...
|---|---|---|
| Personal API/MCP product boundary | Planned / documented | 2026-06-15 routing pass moved the productizable h.computer/creator-new ideas into You.md: You.md owns the personal API/MCP, structured identity/context contract, connected source memory, scoped grants, provenance-rich writeback, YouStacks, host adapters, source refresh, skill learning, and stack-level model routing. h.computer remains Houston's personal site/reference implementation that reads from You.md and writes useful memories/activity back. See `PERSONAL_API_MCP_AND_YOUSTACKS_MEMO_2026-06-15.md` |
| Connected-app grants | Code complete / enforcement pending | Added `connectedAppGrants` with hashed `yg_` tokens, app type, resource scopes, action scopes, write policy, trust level, expiry, revocation, list/page/revoke/create/resolve mutations and tests. Next: wire `yg_` bearer-token auth into HTTP/MCP and agent activity logging |
| Lovable-simple connector UX | MVP code complete | Sources pane now supports connector choices (Website/GitHub/RSS/OKF/Webhook/JSON), crawler provider intent (`native`, `firecrawl`, `agent-browser`, `manual`), refresh cadence, visibility, trust, and optional label. Firecrawl/agent-browser are saved as provider intent only until server-side runner boundaries are implemented |
| Source refresh and monitored updates | Partial code complete | Immutable raw-source versioning and provenance work gives the backend substrate. This slice added source refresh metadata and an hourly cheap cron that marks due sources `pending` without running expensive crawlers/LLMs. Remaining: source detail actions, source health, monitored change summaries, approval/writeback, native/Firecrawl runner |
