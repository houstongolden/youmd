...
| Shell polish pass | Done local / prod visual pending | Logged-in shell/dashboard surfaces hide the global top nav, the left sidebar auto-compacts when the split needs room, chat width is protected by pixel and percentage clamps, sidebar rows no longer use bulky bordered pills or permanent subtext, and account/usage/settings/signout/theme controls now live in a bottom sidebar popout. Production visual approval remains pending |
