...
| Machine bootstrap skill | One-command prompt generation + authenticated web shell proof verified / real new-host run pending | Added bundled `machine-bootstrap`, `youmd machine projects`, and now `youmd machine prompt` so a fresh computer can install/auth/pull/sync You.md, restore shared agent skills/stacks, create a Desktop code root such as `CODE_YOU`, hydrate the Portfolio Graph from You.md/GitHub before cloning, create/clone active project repo directories, optionally restore an encrypted env vault, rehydrate local README/project-context/env-key evidence, and start resident sync daemons. The signed-in shell now handles `/new computer`, `/new machine`, and obvious natural-language setup requests by minting a 7-day scoped bootstrap key and returning a single copyable Claude Code/Codex command while keeping raw `.env.local` values out of the browser prompt. The project command merges authenticated GitHub repos pushed in the last 90 days with You.md project records, uses repo names as directory names, tags associated stacks, attaches API/MCP docs links, skips broad duplicate bundle entries, asks before older projects in interactive mode, supports dry-run/directory-only modes, and clones with `gh repo clone` or `git clone`. Verified locally with focused CLI tests/build, root typecheck, docs check, lint/radius, production build, compiled CLI prompt smoke, and authenticated browser QA of `/new computer` at `http://localhost:3100/dashboard`; screenshot: `/tmp/youmd-new-computer-prompt-2026-06-17.png`. Next: run the generated command on the actual new machine/clean host, add dedicated machine/computer readiness confirmation, and reseed hosted `/api/v1/skills` |
