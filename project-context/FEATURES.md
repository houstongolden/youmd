...
|---|---|---|
| Project portfolio graph and reuse catalog | Planned / product context captured | Added `PROJECT_PORTFOLIO_GRAPH_AND_REUSE_PRD_2026-06-17.md` to define the next project intelligence layer: project goals, AI summaries, PRD/tasks/design/research/ideas docs, API/MCP ownership, protected product agent harness boundaries, public/installable skill stacks, dependency tiers, integration types, failure modes, machine readiness, duplicate-risk warnings, and reusable code/UI/auth/role/sidebar/chat/streaming/connector patterns. First concrete duplicate-risk audit: Lempod ownership across `bamfsite` and `bamfaiapp`. Next: data contract, auditor skill/CLI, dashboard portfolio view, and MCP/API startup slice |
