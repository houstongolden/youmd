...
| Shell opening intelligence brief | Done local / deploy pending | New blank shell sessions now use available real Convex context and ask the You Agent to greet the user by display name, acknowledge concrete recent activity/milestones from bundle version, projects, memories, saved sessions, and repo mirror status when loaded, then suggest useful next moves. A deterministic local fallback brief keeps the transcript useful if the streamed model returns a stub. New Chat now triggers the richer greeting, and half-empty restored sessions with no assistant reply are skipped so a stray `repo` command cannot become the blank shell state |
...
| Shell right-pane responsiveness | Done deployed / visual approval pending | Desktop shell split gives the right detail pane a real minimum width, clamps stored over-wide chat ratios, and keeps the left sidebar/top actions/right detail split mounted at `md` widths. The profile detail pane now renders the compact actionable inspector instead of the full public profile page, and the split has explicit flex shrink/overflow containment so right-pane tabs/content cannot push beyond the viewport. Vercel deployment `dpl_8ygDyGiGWCyz97F1ZQuG2WtJQQzH` reached Ready; authenticated production QA confirmed no horizontal overflow at 900px with 56px rail / 460px chat / 8px handle / 376px detail and `rightEdge: 900` |
...
| Shell logo/sidebar toggle polish | Done deployed / visual approval pending | The separate Lucide left-sidebar toggle beside the YOU mark is removed. The smaller PixelYOU mark is now the sidebar toggle itself, and hover subtly swaps the logo into a two-line menu glyph. The right-pane toggle moved out of the left side of the top chrome and into the far-right top chrome using a cleaner minimal side-panel glyph. Vercel deployment `dpl_DdW1qSmFWKKc8iks3bJxbnaeYjTM` reached Ready; authenticated visual approval remains pending |
| Personal API/MCP stack gateway | Done deployed | Connectors pane now explains the secure personal API/MCP/stack surface, distinguishes `ystack` as the built-in base from the user's private `youstack` and custom `{name}stack`s, shows public/auth/token/share access levels, and frames GitHub/crawlers/grants as source-graph inputs. Architecture note captured in `project-context/PERSONAL_API_MCP_STACK_SURFACE_2026-06-15.md` |
