...
| Agent docs CI guardrail | Done deployed | `npm run agent-docs:ci` runs generated docs checks, generator/smoke script syntax checks, README/root-agent-manual/docs-page handoff marker checks, and targeted lint; the handoff marker check derives the expected root agent-manual CLI version from `cli/package.json`, so `AGENTS.md` and `CLAUDE.md` must stay aligned with the real package version. `.github/workflows/agent-docs.yml` runs that check on path-scoped push, pull request, or manual triggers for agent-docs source surfaces including README, `AGENTS.md`, `CLAUDE.md`, and `/docs#agent-docs`. GitHub Actions run `26852875154` passed and production deployment `dpl_Phut9JEZc3ZuhhPi3mfXZ2FX9Umx` is live |
