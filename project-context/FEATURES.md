...
| DSI catalog components | First private weather/surf slice code complete | Added `dsiComponents` as the first extensible structured component table for the personal API/MCP layer, plus an owner-gated weather/surf refresh action that ports the h.computer Open-Meteo weather and Venice Breakwater surf calibration into private source-backed components. `/shell` Reports now shows a DSI catalog panel with counts, trust/visibility metadata, refresh control, and read-only markdown artifacts under `dsi/private/*`. Remaining: owner-approved public profile rendering, custom component schema/editor, GitHub project catalog with LOC/LOMB/recent updates, BAMF social analytics, Bad.app fitness, school logistics, and research/blog source trackers |
