...
**Status:** DEPLOYED / LOCAL+PRODUCTION CORE VERIFIED / AIR REFRESH + MAC MINI FOLLOW-UPS REMAIN
...
**Production Verified:** YES for docs/API/profile/storage, authenticated `/shell` visual proof, real-model orchestrator smoke, live MacBook Air two-host spawn/launchd proof, CLI `youmd@0.9.4`, and source-Mac remote maintenance self-dispatch. Remaining host-local follow-ups: refresh the Air runtime/daemons to close its stale Skill Mesh catalog row, and refresh the Mac mini runtime/daemons when that machine is available.
...
5. Leave only true external blockers explicit. **DONE, updated 2026-07-01:** the MacBook Air closed the live two-host spawn proof; CLI `0.9.4` added remote maintenance actions and self-dispatch proof on the source Mac; remaining blockers are host-local Air refresh for Skill Mesh catalog convergence and Mac mini runtime/daemon refresh when that hardware is available.
...
**Status:** DEPLOYED / PROFILE + AUTHENTICATED SHELL LIVE-SMOKE VERIFIED
**Verified:** `npx tsc --noEmit`, `git diff --check`, `npm run build`, local production `/houstongolden` smoke (`200`, ~94 KB, one `asciiPortrait` reference, no `Application error` markers), authenticated local `/shell` HTML smoke with zero matches for removed demo strings (`Demo response`, `Houstons-Mini`, `cloud-vps`, `Lock desktop UI`, `Slack capture`, `Connect Cursor cloud`, `Connect Codex cloud`) plus no top-bar mock `3` count, Vercel deployment `dpl_3CMkcND4yZpZ8MiVG8Zp4Dg5TBzD` completed, GitHub build completed success, and live `https://www.you.md/houstongolden?smoke=c80ccb8` returned `200`, ~97 KB, one `asciiPortrait` reference, and no app-error markers. Follow-up production deployment `dpl_9UrW7btDtfSAHMd9jsdHwPmBCY73` for `94a2d56` is Ready and aliased to `https://www.you.md`; GitHub Actions CI run `28487402599` passed; authenticated Chrome QA on `https://www.you.md/shell` verified Houston's shell loads, the sidebar expands with Home/Projects/Skills labels visible, and real session-derived machine counts render as `5` instead of `0`.
...
4. Production proof. **DONE:** deployed and live-smoked public profile; production `/shell` unauth correctly redirects to sign-in; authenticated Chrome QA verified shell render, sidebar expansion, and real machine counts after the session-derived count fix.
...
**Status:** INSTALL-TIME LOCAL INVENTORY WIRED / CLI 0.8.11 PUBLISH TARGET READY / CONVEX+API+CLI SYNC PERSISTENCE DONE / GITHUB SNAPSHOT PROVEN / MCP EXPOSED / MACHINE PROOF NOW LINKS SKILL MESH / FIRST APP SKILL MESH SURFACE BUILT / SOURCE GROUPING STRIP ADDED / LIVE TOPOLOGY GRAPH ADDED / PROOF MATCHING FIXED / RESIDENT INVENTORY REFRESH WIRED / SYNCED INVENTORY STATUS COMMAND DONE / INVENTORY PROGRESS+TIMINGS DONE / API+MCP+APP DRIFT MODEL DONE / RESIDENT DRIFT REPAIR LOOP DONE / ONE-COMMAND MACHINE SYNC DONE / CURL AUTO-SYNC DEFAULT DONE LOCAL / AIR REMOTE SPAWN PROVEN / REMOTE MAINTENANCE ACTIONS LIVE / AIR SKILL CATALOG PROOF PENDING / MAC MINI STILL OPEN
**Verified:** YES locally (`node scripts/local-agent-stack-inventory.mjs`, shared skill script proof under `/tmp/agent-stack-inventory-proof`, `youmd skill list`, `youmd skill use agent-stack-inventory`, shared-agent sync mirror check, compiled `youmd skill inventory`, compiled `youmd skill inventory diff`, compiled `youmd skill inventory status` human/JSON smokes, bounded `youmd skill inventory --out-dir ~/.youmd/agent-stack-inventory --workspace /Users/houstongolden/Desktop/CODE_2025/youmd --sync` proof with `430` unique skill names / `827` real skill files / `secretValuesExposed: false`, compiled progress smoke showing `9` `phaseTimings` records plus bounded `walkIssues` and `secretValuesExposed: false`, CLI build, focused CLI tests, install route regression test, live apex/www installer route checks, temp-home source-channel installer smoke installing `youmd 0.8.11` with `machine sync-now` / `machine full-sync`, bundled scanner smoke, `npm pack --dry-run`, Convex codegen, focused Convex inventory persistence test, focused repo snapshot test, focused MCP registry/harness tests, focused Convex drift tests, focused CLI realtime repair tests, compiled `youmd machine help` / generated fresh-machine prompt smokes for `machine sync-now`, `youmd skill inventory --sync` unauthenticated smoke, CLI prepublish gate, root TypeScript, `docs:check`, `agent-docs:ci`, `youmd machine verify` bounded CLI smoke with skill mesh proof and `secretValuesExposed: false`, focused app eslint, app TypeScript, production Next build, authenticated localhost browser proof for `/shell?tab=skills&view=mesh` source groups/live topology/proof coverage showing `machine proof 4/4`, and catalog-mode proof for `/shell?tab=skills` with no console errors or failed responses)
**Production Verified:** PARTIAL (Convex production deploy succeeded; live production `youmd skill inventory --sync` updated the `Houstons-MBP.lan` row and synced repo snapshot files. GitHub read-back confirms `agent-stack/README.md`, `agent-stack/inventory.md`, and `agent-stack/inventory.json` in `houstongolden/houstongolden-you-md`; `inventory.json` reports `429` unique skill names, `12` You.md catalog skills, `417` catalog gaps, `73` DRY review cases, and `secretValuesExposed: false`. Hosted MCP `tools/list` on `https://www.you.md/api/v1/mcp` exposes `get_agent_stack_inventory`; authenticated `tools/call` returns the latest safe inventory with `429` unique skill names, `826` unique real skill files, `409` direct exposure records, `417` catalog gaps, `73` DRY review cases, `133` mirror clusters, and `secretValuesExposed: false`. Follow-up mirror refresh now includes `agent-stack/README.md`, `agent-stack/inventory.md`, and `agent-stack/inventory.json` from the server-side repo mirror with `missingPaths: []`. The latest local standard-dir sync now records `430` unique skill names, `827` real skill files, `418` catalog gaps, `73` DRY review cases, and `secretValuesExposed: false`, and `youmd skill inventory status` lists the synced rows with count drift. On 2026-07-01 hosted Convex API/MCP smokes for agent-stack inventory and synced-brain graph passed with `secretValuesExposed: false`; `Houstons-MacBook-Air.local` also proved live remote daemon/spawn behavior. CLI `youmd@0.9.4` now exposes remote `skill.inventory` and `machine.verify`, and source self-dispatch updated `MacBookPro.lan` to `434` unique skill names / `1,175` real skill files / `431` catalog skills / `4` gaps with `secretValuesExposed: false`. The remaining Air Skill Mesh proof is specifically the synced inventory row still reporting `youmdCatalogSkills: 0`; source-side post-0.9.4 remote attempts timed out until the Air's local runtime/daemon is refreshed. App/Tauri richer drilldowns and Mac mini proof remain open)
...
4. Identify the sync gap before changing behavior. **DONE first pass + updated after shared-skill registration:** this Mac now has `427` unique local skill names, `824` unique real `SKILL.md` files, `409` direct host exposure records, and `12` You.md catalog skills; installed global `youmd` was `0.8.2`, npm latest reports `0.8.7`, and repo CLI is now `0.8.11`.
...
9. Prepare the CLI publish target so the Mac mini can install the new commands through normal `youmd` update/install. **DONE locally / SOURCE CURL PROVEN / NPM PERMISSION BLOCKER:** CLI package is bumped to `0.8.11` for the inventory commands plus resident Skill Mesh refresh; full CLI tests, CLI build, and package dry-run pass locally. A temp-home curl/source smoke installed `0.8.11` from GitHub `main` without sudo and showed `youmd sync --live --daemon` help. Trusted publish runs now pass tests/build and produce signed provenance, but npm still rejects `youmd@0.8.11` with `E404 / no permission`, so npm package Trusted Publishing/package permissions remain the external gate. **PENDING:** npm package permission fix and Mac mini install proof.
10. Productize install-time inventory in `install.sh` / machine setup so a new Mac automatically inventories and verifies the skill/stack system after install. **DONE local report + Convex/GitHub/MCP/machine-proof sync + one-command repair + curl auto-sync default slice:** CLI package bundles the scanner, curl install writes best-effort safe inventory, fresh-machine bootstrap runs inventory after shared skills/MCP restore and near final proof, and both paths call `you skill inventory --sync` so authenticated installs persist a safe summary and attempt the repo snapshot path. `install.sh` now defaults `YOU_INSTALL_MACHINE_SYNC` to `auto`, tries `YOU_API_KEY`, existing local auth, or browser device login through `/dev/tty`, then runs `you machine sync-now` automatically; `YOU_INSTALL_MACHINE_SYNC=0` is the explicit local-only escape hatch. `you machine verify` now auto-loads the latest local inventory and writes skill mesh counts/report paths into the machine proof JSON. `you machine sync-now` now collapses identity/shared-stack/skill/MCP/inventory/proof/daemon repair into one command, and `you machine full-sync` adds active project clone + Secret Vault pull attempts. The scanner is bounded with `walkIssues` metadata and streams secret-safe progress phases plus `phaseTimings` into JSON/HTML/terminal output so install-time scans are diagnosable instead of silently hanging. **OPEN:** deploy/source-curl proof + second-machine proof.
11. Add Convex persistence for safe machine inventory snapshots, sync events, drift summaries, and DRY review queues. **DONE local backend/API/CLI slice:** `agentStackInventories` stores safe counts, ownership/sync/provenance rollups, catalog-gap samples, DRY-review samples, mirror samples, local report paths, and linked brain activity rows. API routes post/list summaries; CLI `--sync` writes them when authenticated.
12. Add GitHub repo snapshots under the user's `username-you-md` repo for portable markdown/JSON machine/skill/stack inventory. **DONE + PRODUCTION PROVEN:** identity repo pushes now include generated `agent-stack/README.md`, `agent-stack/inventory.md`, and `agent-stack/inventory.json` from safe Convex summaries. Inventory API sync attempts repo push/mirror by default, and live GitHub read-back verified the three `agent-stack/` files plus safe counts and `secretValuesExposed: false`.
13. Expose inventory topology through You.md API and MCP so authorized agents can inspect and repair drift. **DONE drift slice:** authenticated API post/list routes exist for safe summaries; local stdio MCP and hosted JSON-RPC MCP expose `get_agent_stack_inventory`; generated docs advertise the hosted inventory tool; hosted MCP reads the mirrored `agent-stack/` repo snapshot files; `GET /api/v1/me/agent-stack/drift` now returns baseline, machine count deltas, stale/unsafe flags, issue labels, and repair commands; MCP includes drift by default with `include_drift` opt-out. **OPEN:** optional MCP resources for topology/report files.
14. Build the web/Tauri Skill Mesh surface for diagram/report/ownership/DRY/catalog/machine drift views. **DONE first app + drift + graph + source grouping slice / broader surface open:** Skills now has a `skill mesh` mode backed by Convex `agentStackInventories` plus `getAgentStackInventoryDrift`, showing a real-data topology graph, machine counts, source groups for Houston managed / GStack reference / public upstream / runtime plugin / needs review, ownership/sync/provenance rollups, catalog gaps, DRY queues, mirror samples, machine snapshots, drift baseline/summary, per-machine status labels, and verify/compare/refresh commands. Machine readiness now adds a graph-first synced brain view backed by real local readiness, synced machine proofs, Convex `brainActivities`, daemon status, agent bus, skill mesh, project graph, and Secret Vault metadata. **OPEN:** richer drilldowns, dynamic graph in Skills/Home/Tauri, and Tauri-native packaging.
15. Add resident realtime reconciliation so skills/stacks/context metadata stay synced across trusted machines and agents. **DONE first repair slice / broader product loop open:** `youmd sync --live --daemon` now runs bounded inventory refresh every 30 minutes by default, checks the local host's drift row against the best complete safe baseline, and when stale/divergent/unsafe runs `youmd pull`, `youmd stack sync`, `youmd skill sync`, inventory sync, and machine proof sync. Env knobs exist for disabling/tuning inventory and repair. `youmd skill inventory status` lists the latest local proof, synced machine inventory summaries, count drift, and exact refresh/verify/diff commands for Mac mini comparison. **OPEN:** richer repair receipts/UI, optional topology MCP resources, and second-Mac proof.
...
**Progress (2026-07-01 remote maintenance / CLI 0.9.4):** Added and published first-class remote `skill.inventory` and `machine.verify` actions so trusted hosts can repair Skill Mesh and machine proof drift without pasting raw shell commands. The executor now builds fixed argv for each action, requires target-host opt-in, rejects arbitrary roots for `machine.verify`, and exposes the contract through remote help plus local MCP schema. Verification passed with CLI build, focused `remote-executor` tests (`30` tests), `npm run agent-docs:ci`, `npm pack --dry-run`, live `llms:smoke`, green CI `28501882155`, green Agent Docs `28501882183`, and trusted npm publish workflow `28501882181`. `youmd@0.9.4` is live on npm and installed on the source Mac. After safely reconciling active `~/.you` auth config with legacy `~/.youmd` config and reloading launchd, self remote `agent.status` request `rc_01KWEAJCCB000GM3JCT7S` returned in ~2s and self remote `skill.inventory` request `rc_01KWEAJPP50002M9MM2K4` returned in ~29s with `secretValuesExposed: false`, `434` unique skill names, `1,175` real skill files, `431` catalog skills, and `4` gaps. Current blocker: `Houstons-MacBook-Air.local` did not answer post-0.9.4 `skill.inventory` request `rc_01KWEAN93J0002YFA726J` within 60s or `agent.status` request `rc_01KWEAQBV6000G2ZT2KJT` within 45s, so the Air needs local runtime/daemon refresh before source-side remote maintenance can close its stale catalog row.
**Progress (2026-07-01 Air catalog convergence):** After Houston's Air repair block, launchd remote-command consumption recovered (`agent.status` request `rc_01KWE8F8H20001EY40S95` returned from `Houstons-MacBook-Air.local`), and production Convex API/MCP smokes for agent-stack inventory plus synced-brain graph passed safely. The Air inventory row, however, still synced `youmdCatalogSkills: 0` because the first `--register-catalog` run can write/hydrate the catalog after generating the inventory JSON, then upload that pre-registration snapshot. Fixed the CLI wrapper so `you skill inventory --register-catalog --sync` reruns inventory once when catalog hydration/registration increases the catalog count and skips sync if the post-catalog proof cannot be generated. Verified with CLI build, focused skill-catalog tests, and a temp-home compiled CLI smoke that converged from `catalog: 0 / missing: 1` to `catalog: 12 / missing: 0` in one command. Published CLI `youmd@0.9.3` through trusted workflow `28501017652`, regenerated docs/OpenAPI and root handoff markers, refreshed this Mac's runtime, and reloaded resident daemons. Superseded by `youmd@0.9.4`, which adds source-side remote `skill.inventory`; remaining live proof is to install/reload `0.9.4+` on the Air and rerun inventory until its synced row no longer reports `youmdCatalogSkills: 0`.
**Progress (2026-06-22 curl bootstrap):** Updated the generated curl installer so the one command is the default machine setup orchestrator. It now source-installs/migrates/inventories/configures MCP as before, then authenticates via `YOU_API_KEY`, existing local auth, or browser device login through `/dev/tty`, and runs `you machine sync-now` automatically unless disabled with `YOU_INSTALL_MACHINE_SYNC=0`. Focused installer Vitest, CLI TypeScript build, and production Next build passed. Remaining proof is deploy/main curl smoke plus second-Mac verification.
...
2. Surface the characters where the product magic is happening, especially machine/agent sync. **DONE first slices:** Machine `you sync mesh` host chips/metrics, Live Brain Log rows, and connected-agent activity rows now show machine/agent/shell characters tied to real readiness/activity signals.
...
5. Make Home a DSI default View with widgets for chat/shell, live log, tasks, projects, blockers, shipped work, machines, agents, skills, and vault state. **DONE first persisted slice:** default Home view stores You Agent chat/shell, Live Brain Log, Needs Houston, Agent Queue, Project Focus, and Machine Mesh widgets.
6. Add Convex/realtime-backed `views` and `widgets` data model plus security contracts. **DONE first slice:** `dsiViews` and `dsiViewWidgets` with layout/query/security metadata and `rawSecretsInBrowser: false` Home proof.
...
**Progress (2026-06-20 minimal-surface graph pass):** Machine readiness now makes the mesh graph-first: trusted Macs, agents, skills, projects, Secret Vault, and the You.md brain stream are rendered as status-linked nodes with real links from local readiness/proofs/Convex `brainActivities`. Nodes only pulse when backed by recent timestamps or active/ready proof state, and the older linear mesh strip was removed so the graph is the primary surface instead of another stacked table.
...
5. Make the actual brain graph the canonical source for all project/skill/source/task/sync events. **DONE expanded durable + navigable slice:** owner-gated Convex `brainActivities` is the canonical stream; new realtime agent-bus messages mirror into it; shell Live Log reads it first; repo update runs/steps, portfolio task changes/triage/detail edits, brain-dump captures, extracted brain-dump tasks, skill syncs, shared stack syncs, skill outcomes, skill improvement analysis, richer cross-agent coordination messages, portfolio hydrations, source-crawl policy decisions, realtime daemon checkpoints, machine proofs, GitHub repo-mirror refreshes, Secret Vault registrations/uploads/downloads, and trusted-device envelope shares/pulls write directly into it with `secretValuesExposed: false`. Live Brain Log rows now preserve safe project/entity/host/agent metadata and render compact click targets into project detail, Skill Mesh, Machine proof, Tasks, Settings, or Activity surfaces.
...
7. Make the Machine page foreground connected computers and real sync rather than a one-time setup checklist. **DONE local slice:** the setup block collapses into a compact row, and Machine now opens onto a `you sync mesh` with real this-Mac, trusted-machine, agent-bus, skill-mesh, project-graph, and Secret Vault signals.
...
**Progress (2026-06-19 foregrounding follow-up):** Combined the hidden Activity and Stats surfaces with Live Log by adding compact `AGENTS` and `STATS` rows from connected-agent summaries and profile analytics. Reordered Settings so the second-brain GitHub repo and update history appear in the first viewport; API key scopes, active key rows, and session logs are truncated behind reveal controls. Codex in-app Browser screenshots verified Settings repo/update history above the fold and Live Log `AGENTS`/`STATS` rows.
**Progress (2026-06-19 Machine mesh follow-up):** Made the Machine tab's new-computer setup panel collapsible and persisted that preference in the browser, leaving a compact `new machine setup` row with `copy setup` and `details`. Added a foreground `you sync mesh` that uses real local readiness + Convex proof data for this Mac, trusted machine count/hosts, realtime agent-bus state/messages/channels, shared skill mesh status/counts, project graph/env/context counts, and Secret Vault snapshot readiness. Codex in-app Browser visual proof verified expanded setup, collapsed setup, and hydrated mesh values (`Houstons-MBP`, `6` trusted machine proofs, active agent bus, ready skill mesh, `26/61` project graph, ready Secret Vault) without exposing secrets.
**Progress (2026-06-19 sidebar/light follow-up):** Simplified the left chat history to a standard always-visible one-line list: `CHATS` label only, no section icon, no group toggle, no row icons, no subtitles, and first 50 sessions queried. Added first-prompt title fallbacks for new sessions and Convex backfill from saved chat messages for older sessions without summaries. Softened light mode with warm beige design tokens and a subtle radial/linear gradient; Codex in-app Browser proof on `/shell?tab=apis` confirmed the page reads as muted ivory/beige instead of stark white.
**Progress (2026-06-19 shell color-rule follow-up):** Replaced near-black/orange shell active fills and top chrome with warm neutral shell tokens: top GitHub chrome now reads as warm charcoal, chat/machine active fills use warm charcoal instead of orange wash over black, and shared callout gradients fade into warm shell surfaces. Added the durable design rule to `STYLE_GUIDE.md` and `BRANDING.md`: no orange gradient over solid black shell surfaces; use orange as a thin accent/status/label only.
**Progress (2026-06-19 producer follow-up 2):** Added GitHub repo-mirror activity rows and Secret Vault pull/download activity rows as durable `brainActivities` producers. Mirror refreshes now upsert `github:repo-mirror:<repo>` rows with stale/synced status, commit/file/byte metadata, and no secret exposure. Trusted-device envelope pulls and encrypted snapshot downloads now emit vault rows with safe counts, filenames, hosts, and short checksums only; raw vault archive bytes, wrapped passphrases, and full hashes stay out of the browser/log stream. Focused Convex tests prove both producer paths and redaction.
**Progress (2026-06-19 CLI shared-stack follow-up):** Upgraded `youmd skill sync` to emit richer shared-skill activity rows and added `youmd stack sync` activity rows for canonical `~/.agent-shared` propagation. The payloads include safe shared skill counts/names, source/scope rollups, Stack Map presence, git head/branch/dirty state, host/runtime info, and `secretValuesExposed: false`; they do not store command logs, skill contents, API keys, or env values. CLI is bumped to `0.8.9` for npm publish readiness.
**Progress (2026-06-19 skill-improvement follow-up):** Routed API/MCP skill outcome writes and `youmd skill improve` proposal analysis into `brainActivities`. Outcome rows store status, skill name, runtime/agent, duration, note presence/length, and `noteStoredInActivity: false`; improvement rows store proposal counts, safe proposal summaries, low-performer skill metrics, host/runtime info, and `secretValuesExposed: false`. Focused Convex and CLI tests prove notes, identity values, API keys, and env-like values are not copied into the browser-visible activity stream.
**Progress (2026-06-19 agent-coordination follow-up):** Upgraded realtime agent-bus activity rows to infer and store coordination links: `projectSlug`, `skillName`, `entityType`, `entityId`, related projects, and related skills. Local agents can now pass explicit links with `youmd agent send --project <slug> --skill <name> --entity-type <type> --entity-id <id>`, while the server also infers common labels like `project: youmd`, `skill: machine-sync`, and `task: env-restore` from redacted message text. Focused Convex coverage proves project/skill/entity links and secret redaction; a live local-CLI smoke sent `am_6Sc4fUAIm2B5BKU26EDTIISd` for `youmd / machine-sync / task:agent-bus-link-proof`.
**Progress (2026-06-19 shell-layout follow-up):** Added persisted desktop layout controls for the shell chat/live-log column: hide/show chat, flip it left/right of the detail pane, and resize it to a compact column. The shell prevents a blank workspace by forcing chat visible when the detail pane is closed. Verified with root TypeScript, production Next build, and live localhost `/shell?tab=tasks`.
**Progress (2026-06-20 Live Log filter follow-up):** Added compact `all` / `agents` / `ops` / `skills` filters to the shell Live Log. The filters use existing safe entry metadata from Convex `brainActivities` and local readiness supplements, so the central log stays one surface while agent coordination, machine/vault ops, and skills/projects can be inspected separately.
...
8. Route machine/vault setup proof into the same pulse. **DONE DEPLOYED:** machine proof refreshes, Secret Vault trusted-device registration, encrypted env snapshot upload, and trusted-device envelope share/refresh now write `brainActivities` without public keys, wrapped passphrases, encrypted archive bytes, manifest text, or raw env values.
9. Remaining architecture gap. **OPEN:** GitHub-backed shared skill repos still use conflict-safe git sync as the materialization layer; true instant cross-device sync for every non-Convex source should add GitHub webhook/change-event ingestion into the Convex sync head.
...
**Progress (2026-06-20):** Continued the minimal-surface product pass by extracting the real synced brain graph renderer into `src/components/sync/SyncedBrainGraph.tsx` and the shared data adapter into `src/components/sync/machineBrainGraphModel.ts`. Machine and Home now use the same graph model for local readiness, synced proofs, Convex `brainActivities`, agent bus, skills, projects, daemons, Secret Vault metadata, and optional portfolio context. Home replaces its older mini mesh stat strip with the real graph, reducing duplicate status surfaces while keeping Machine as the detailed proof/runbook page. Follow-up perf pass cached/deduped `/api/local/machine-readiness`, split passive Machine load from manual forced refresh, slowed passive shell polling to 60s, and proved authenticated localhost repeat calls dropped from a 5.56s `miss` to 0.50s/0.40s `fresh` reads. Authenticated browser proof passed for Home, Machine, and Skills mesh surfaces: real graph/mesh text hydrated and no console errors were captured.
**Progress (2026-06-20 minimal-surface pass):** Skill Mesh now opens with the real synced brain graph plus three decision signals instead of a long stack of graph, latest-machine stats, source groups, drift, proof, rollups, samples, snapshots, and commands. Source grouping, machine proof, and audit tables are preserved behind focused detail modes (`source map`, `machine proof`, `audit details`). Verified with root TypeScript and lint/radius check for the touched file. Next product slice remains open: move graph liveness into a single Convex-backed DTO/query that joins `agentStackInventories`, `machineProofReports`, `brainActivities`, and portfolio signals.
**Progress (2026-06-20 Skill Mesh minimal-surface audit cleanup):** Refined Skill Mesh into a cleaner four-mode structure (`overview`, `source map`, `machines`, `audit details`) and kept machine parity in the overview. The live graph now shows a compact Convex evidence/freshness strip for inventories, proofs, recent activity, and projects. Detailed drift/proof rows moved into the `machines` mode, where drifted or stale rows now reveal bounded copyable repair chains sourced from Convex drift metadata plus a localhost-only bridge for whitelisted `you` repair/proof actions on the matching local Mac. The local HTML report action was folded into the existing `next action` controls to avoid adding another permanent surface.
**Progress (2026-06-20 canonical graph DTO):** Added Convex `getSyncedBrainGraph`, authenticated `GET /api/v1/me/synced-brain/graph`, hosted MCP `get_synced_brain_graph`, generated docs/openapi/llms updates, and a focused portfolio regression test. Machine now prefers the canonical DTO and its first sync mesh viewport has been reduced to the real graph plus three proof facts, with runbook/proof detail still below for repair. Verified with Convex codegen, `npx vitest run convex/portfolio.test.ts`, `npx tsc --noEmit --pretty false`, and `npm run build`. Next slice remains open: make Home and Skills consume the DTO directly and live-smoke API/MCP after production deploy.
**Progress (2026-06-20 app consumer wiring):** Promoted `SyncedBrainGraphDto` into the shared graph renderer contract, then wired Home and Skill Mesh to query `api.portfolio.getSyncedBrainGraph` with portfolio signals before falling back to their previous local/inventory graph builders. Machine, Home, and Skill Mesh now share one canonical persisted graph contract while preserving dev/offline resilience. Verified with root TypeScript and production Next build; production API/MCP smoke remains the next deploy-time proof.
**Progress (2026-06-20 production graph smoke):** Live-smoked the production Convex API base from local You.md config. `GET /api/v1/me/synced-brain/graph?includePortfolioSignals=true&limit=12` returned schema `you-md/synced-brain-graph/v1`, 7 nodes, 8 links, 6 signals, 4 latest activity rows, evidence counts for inventories/proofs/activity/projects/tasks, `secretValuesExposed: false`, and no detected API-key/env/private-key text. Hosted MCP `tools/call get_synced_brain_graph` returned the same safe shape. Also updated MCP discovery/auth resource copy so the new graph tool is listed with private read tools.
**Progress (2026-06-20 repeatable graph smoke):** Added `scripts/smoke-synced-brain-graph.mjs` and `npm run sync:graph:smoke`. The script reads `YOU_API_KEY`/`YOUMD_API_KEY` or local `~/.you`/`~/.youmd` config, calls production REST plus hosted MCP, prints only redacted schema/count/evidence/safety summaries, verifies REST/MCP parity, and exits nonzero on missing auth unless `--allow-missing-auth` is passed. Text and JSON smoke runs passed against production with `secretValuesExposed=false` and `unsafeTextDetected=false`.
**Progress (2026-06-20 graph minimal surface):** Made the shared synced brain graph selectable with a compact selected-node inspector and active-link pulse markers that render only for real active graph links. Machine now hides proof/repair commands and agent-bus/skill-mesh diagnostics behind focused disclosure, keeping the first viewport centered on useful liveness/proof instead of raw audit clutter. Verified with root TypeScript, production Next build, clean generated-docs check, production graph smoke, and `git diff --check`.
**Progress (2026-06-20 hosted report resources):** Added hosted MCP `agent-stack://inventory/report.html`, generated from persisted safe inventory/drift/repo mirror metadata, plus `npm run sync:agent-stack:smoke` to verify agent-stack REST summaries, hosted MCP `get_agent_stack_inventory`, `resources/list`, resource summary, generated HTML, and repo-backed snapshot files without printing raw contents. Local verification passed for TypeScript, agent-docs CI, generated docs baseline, and diff hygiene; production smoke is expected after deploy because the resource is new.
**Progress (2026-06-20 app report access):** Added signed-in localhost-only `GET /api/local/agent-stack-report` to open the newest generated local Skill Mesh HTML report from `~/.you/agent-stack-inventory` or legacy `~/.youmd/agent-stack-inventory`. Skill Mesh now exposes that report through a compact `open local HTML report` action inside the existing verify/compare area, alongside the hosted MCP `agent-stack://inventory/report.html` URI, so users can see the full visual inventory without another permanent dashboard panel. Verified with root TypeScript, production Next build, and clean-reference agent-docs CI.
**Progress (2026-06-20 dynamic report mode):** Added a default `report` mode in Skill Mesh that turns the synced inventory into a compact decision surface: source-of-truth ownership, sync health, catalog coverage, DRY review pressure, samples, and the single next useful command. It reuses existing safe source-group/proof/drift semantics and keeps source map, machine proof, and audit tables behind focused modes. Verified with root TypeScript, production Next build, and clean-reference agent-docs CI.
**Progress (2026-06-20 Skill Mesh activity drilldown):** Skill Mesh report mode now queries canonical `brainActivities`, filters for skill/inventory/machine/daemon/vault/sync evidence, and embeds a compact Live Brain Log with the same safe project/entity/host/agent click targets as the main stream. URL-backed lanes (`activity=skills|machines|vault|sync`) make that evidence shareable/directly openable from the app. This connects the topology/report counts to real recent sync/proof activity without rendering raw skills, prompt logs, `.env.local`, tokens, or vault secrets. Verified with root TypeScript, production Next build, and clean-reference agent-docs CI.
...
## 2026-06-25 — Cross-machine agent work-context sync (the You.md core problem, internal)

### 145. Give cross-machine agents real-time shared context about in-flight work (branches, worktrees, intent)
**Status:** OPEN / DESIGN
**Source:** 2026-06-25 — Houston, after an agent on this MacBook refused to merge two unmerged remote branches (`claude/desktop-demo`, `claude/youmd-agent-secure-setup-n2upam`) citing "zero knowledge of what they are / why": *"this is indicative of the problem we are trying to solve with you.md to begin with ... two agents working on two different computers SHOULD have that knowledge and context about what the other is working on ... refusing to merge bc of [no knowledge] is just going to create more problems the longer the agents on the two different computers do not get in sync ... whether it is added knowledge/context real-time sync via convex for branches/worktrees or whatever else like this to help agents better collaborate on different machines in sync."*
**Why this matters:** This is You.md's own dogfood failure. The cross-machine **command** bus (`you remote run`, daemons 4/4) already works, but there is no cross-machine **work-context** layer. An agent on machine B sees another machine's branch as an opaque ref with no PR, no task summary, no rationale — so it either refuses (blocks progress) or merges blind (breaks main). The longer two machines' agents stay context-blind, the more divergence/conflict accrues (e.g. the secure-setup branch is now 9 days stale and conflicts with main on `convex/schema.ts`/`http.ts`/`cli api,index`).
**Goal / Success Definition:** Any agent on any synced machine can, before branching/merging, answer "what is in flight elsewhere, by whom, and why?" from a live, secret-safe, Convex-backed work-context graph — and can act on it (informed merge, rebase, or hand-off) without re-deriving it from raw git.
**Actionable Scope (proposed, additive — no user homework):**
1. **Publish leg:** extend the resident realtime daemon to publish a secret-safe per-machine work snapshot to Convex: active branch, worktrees, ahead/behind, recent commits, dirty state, detected open PRs (`gh`), and a short agent-authored "current task + why" intent string. Keyed by machine + agent identity + timestamp.
2. **Convex model:** new table (e.g. `agentWorkContext` / `branchActivity`) alongside the existing machine-proof/`brainActivities` tables; redaction + ownership scoping like the existing proof pipeline.
3. **Read surface:** `you remote work` (and extend `you remote list` / `you remote status`) to render the cross-machine work graph — unmerged branches across all machines with intent, staleness, and conflict-vs-main flags.
4. **Whitelist + MCP:** add read-only `git.branches` / `git.unmerged` to the remote-command whitelist, and an MCP tool (`remote_work_context`) so any agent can query in-flight work before deciding to merge.
5. **Intent journal:** per-branch "why" log synced via Convex so merge decisions carry rationale, not just diffs. Agents append automatically when they start/continue a branch.
6. **Merge-decision helper:** given a target branch, surface PR state + conflict preview + intent so an agent can recommend merge/rebase/hold with evidence instead of refusing for lack of context.
**Immediate trigger to resolve separately (the two branches that exposed this):**
- `claude/desktop-demo` (9870f37) — `git cherry` shows its change is **already in main** (landed via merged PR #26); it is a **stale leftover** → delete, do not merge.
- `claude/youmd-agent-secure-setup-n2upam` (ed90eed) — **real unmerged feature** (+1173 lines: zero-knowledge `.env.local` handoff + `secure-machine-setup` skill/stack), **no PR**, **conflicts with current main** on `convex/schema.ts`/`http.ts`/`cli api,index`; directly overlaps the Secret Vault / cross-machine sync work → needs a proper conflict-resolved merge + Convex schema review + tests, not a blind merge. Best done by whichever agent already holds the context (portability is the point).
