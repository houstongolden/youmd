...
**Status:** IN PROGRESS (production graph-backed planning, readiness audit, and authenticated web/CLI proof complete; actual new-host run + opt-in local server smoke pending)
**Verified:** PARTIAL (`youmd machine prompt` / `youmd machine verify` unit tests/build, root typecheck, docs generation/check, Convex codegen/deploy, production graph dry-run, compiled CLI prompt/readiness smokes, and authenticated local browser `/new computer` QA passed)
...
8. Add a first-class post-clone readiness/run checker. **PARTIAL DONE:** `youmd machine verify` now reports git remotes, package managers/scripts, `.env.local` presence, `.env.example` presence, agent docs, and `project-context/` readiness without reading secret values, and the generated command runs it after clone/env restore. **PENDING:** explicit opt-in run/smoke mode that installs dependencies and probes key local servers safely on the clean host.
