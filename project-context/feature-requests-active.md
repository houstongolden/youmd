...
**Status:** IN PROGRESS (core implementation + local authenticated web proof + CLI/local-agent task/brain-dump proof complete; editing controls, mobile/watch adapters, richer strategy enrichment, and update-artifact history still open)
**Verified:** PARTIAL (root build, Convex tests/typecheck, CLI build, local MCP smoke, shared-skill mirror sync, authenticated Chrome `/shell` repo-update QA, authenticated persisted Portfolio Graph pane QA, authenticated 90-day GitHub project catalog refresh QA, authenticated shell-chat task/brain-dump repo-sync QA, authenticated CLI task/brain-dump repo-sync QA, built MCP registry proof, and authenticated browser verification of CLI-created rows passed)
...
5. Add persisted Convex portfolio graph records instead of static-local data only. **DONE first verified slice:** `portfolioProjects`, `portfolioApiSurfaces`, `portfolioDependencyEdges`, `portfolioReusablePatterns`, `brainDumpCaptures`, and `portfolioTasks` tables plus owner-gated `portfolio.*` queries/mutations. The `/shell` Portfolio Graph pane now queries persisted records, uses static data only as a bootstrap fallback, and authenticated Chrome QA verified `CONVEX PERSISTED GRAPH` with 4 projects, 5 surfaces, 4 edges, and 5 patterns after pressing `refresh persisted graph`. **PENDING:** repo snapshot export and richer strategy-field enrichment.
...
7. Add `PROJECTS => Tasks` with personal/uncategorized tasks and project-associated tasks. **DONE first web slice:** `portfolioTasks.projectSlug` is optional, so uncategorized/personal tasks and project tasks share one table; authenticated Chrome shell-chat QA saved a personal task through `/task me personal: ...`, displayed it as `HUMAN / PERSONAL`, wrote `projects/_personal/tasks.md`, and synced PR #8. **PENDING:** dashboard editing/triage workflow.
...
**Status:** IN PROGRESS (local dashboard/data/CLI/shared-skill/MCP foundation shipped and locally visually verified; persistence + production verification pending)
**Verified:** PARTIAL (local TypeScript, CLI build, focused shared-skill source tests, portfolio audit smoke, env-key audit, shared skill mirror check, local stdio MCP smoke, production build, self-improvement CLI review, local skill install/sync, and signed-in local visual QA passed)
**Production Verified:** NO
