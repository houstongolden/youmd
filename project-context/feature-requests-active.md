...
**Observed Failure:** A user-provided agent prompt produced repeatable server errors for two freshly generated secure tokenized links. The external agent reported request IDs `70c536ff77274a5e` and `10a3d0c0aaa1a2bb`, indicating the endpoint accepted the request and failed server-side.
**Progress (2026-05-21):** Reproduced that the exact link currently resolves with redirect-following, then hardened the fragile server path that could turn valid links into 5xxs: `incrementUseCount`, `recordView`, and activity logging are now best-effort, unexpected `/ctx` exceptions return JSON, and both Convex + Next proxy responses use private/no-store cache headers with `Vary: Accept`. Local production-build proxy verification against the live token confirms JSON has `identity` + `_privateContext`, text/plain still returns markdown, invalid tokens return 404, and ETag revalidation still returns 304. Production deployment pending via push/GitHub Actions because `CONVEX_PROD_DEPLOY_KEY` is not present locally.
**Scope Checklist:**
