...
**Verified:** PARTIAL (`youmd machine prompt` / `youmd machine verify` unit tests/build, bounded package-check tests/build, root typecheck, docs generation/check, Convex codegen/deploy, production graph dry-run, compiled CLI prompt/readiness smokes, authenticated local browser `/new computer` QA, authenticated local browser Machine readiness QA, global CLI refresh, and exact `youmd machine verify`/daemon-status command proof passed)
