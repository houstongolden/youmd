...
**Progress (2026-06-17 active-focus setup gate):** Tightened `youmd machine projects` and generated fresh-computer prompts so the Portfolio Graph is now the default setup authority: when graph records exist, the planner clones only projects with `status: active` and `focusStatus: top-priority` or `focusing`; inactive, unsorted, on-ice, abandoned, killed, and unreviewed GitHub-only repos are skipped unless `--include-inactive` is explicitly supplied. Added manual project status persistence (`active`/`inactive`) with `statusSource: manual`, preserved it through tracked GitHub hydration, and verified the built CLI dry-run against the live graph: `56` projects / `40` tracked repos -> `16` selected / `84` skipped. Focused CLI tests, Convex regression, CLI build, lint/radius, production build, and local authenticated UI read proof passed. Authenticated status-click write proof is pending remote Convex deployment because this shell lacks `CONVEX_PROD_DEPLOY_KEY`.
