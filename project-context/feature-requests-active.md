...
**Status:** IN PROGRESS (production graph-backed planning and authenticated web/CLI proof complete; actual new-host run + local readiness proof pending)
**Verified:** PARTIAL (`youmd machine prompt` unit tests/build, root typecheck, docs generation/check, Convex codegen/deploy, production graph dry-run, compiled CLI prompt smoke, and authenticated local browser `/new computer` QA passed)
...
3. Have the command install You.md, authenticate, pull/sync identity, restore shared skills/stacks/agent config, create `~/Desktop/CODE_YOU`, clone active GitHub-backed projects, restore encrypted env vaults, rehydrate local evidence, install daemons, and show status. **DONE in generated command; production graph-backed clone plan verified; pending real new-host execution and local server/readiness checks.**
...
8. Add a first-class post-clone readiness/run checker. **PENDING:** current command plans and clones; a follow-up `machine verify`/readiness path should install deps or run project-specific smoke checks safely instead of pretending every app can share one generic launch command.
