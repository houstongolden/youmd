...
10. Productize install-time inventory in `install.sh` / machine setup so a new Mac automatically inventories and verifies the skill/stack system after install. **DONE local report + Convex sync slice:** CLI package bundles the scanner, curl install writes best-effort `~/.youmd/agent-stack-inventory`, fresh-machine bootstrap runs inventory after shared skills/MCP restore and near final proof, and both paths call `youmd skill inventory --sync` so authenticated installs persist a safe summary. **OPEN:** persist snapshots into GitHub and show them in app/MCP.
