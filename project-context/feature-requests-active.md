...
10. Productize install-time inventory in `install.sh` / machine setup so a new Mac automatically inventories and verifies the skill/stack system after install. **DONE local report + Convex/GitHub/MCP/machine-proof sync + one-command repair + authed curl handoff slice:** CLI package bundles the scanner, curl install writes best-effort `~/.youmd/agent-stack-inventory`, fresh-machine bootstrap runs inventory after shared skills/MCP restore and near final proof, and both paths call `youmd skill inventory --sync` so authenticated installs persist a safe summary and attempt the repo snapshot path. `install.sh` now also runs `youmd machine sync-now` automatically when `YOUMD_API_KEY` or `YOUMD_INSTALL_MACHINE_SYNC=1` is present, otherwise prints that single next command. `youmd machine verify` now auto-loads the latest local inventory and writes skill mesh counts/report paths into the machine proof JSON. `youmd machine sync-now` now collapses identity/shared-stack/skill/MCP/inventory/proof/daemon repair into one command, and `youmd machine full-sync` adds active project clone + Secret Vault pull attempts. The scanner is bounded with `walkIssues` metadata and now streams secret-safe progress phases plus `phaseTimings` into JSON/HTML/terminal output so install-time scans are diagnosable instead of silently hanging. **OPEN:** npm publish + second-machine proof.
