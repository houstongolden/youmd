...
**Status:** DONE locally (authenticated visual QA passed; production visual approval still welcome)
**Verified:** YES (focused dashboard lint, root lint/radius, `git diff --check`, `npm run build`, and authenticated local `/shell` Playwright QA passed)
...
3. Verify locally in the actual shell UI. **DONE:** minted a real local web session, opened `http://localhost:3100/shell`, clicked the visible `Expand sidebar` control, verified the rail toggled `56px -> 244px -> 56px`, confirmed labels render in expanded state, and saved screenshots `/tmp/youmd-sidebar-expanded-auth-2026-06-17.png` and `/tmp/youmd-sidebar-collapsed-auth-2026-06-17.png`.
