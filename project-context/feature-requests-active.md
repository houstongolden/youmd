...
10. Productize install-time inventory in `install.sh` / machine setup so a new Mac automatically inventories and verifies the skill/stack system after install. **DONE local report + Convex/GitHub sync slice:** CLI package bundles the scanner, curl install writes best-effort `~/.youmd/agent-stack-inventory`, fresh-machine bootstrap runs inventory after shared skills/MCP restore and near final proof, and both paths call `youmd skill inventory --sync` so authenticated installs persist a safe summary and attempt the repo snapshot path. **OPEN:** prove repo snapshot in production and show it in app/MCP.
...
13. Expose inventory topology through You.md API and MCP so authorized agents can inspect and repair drift. **DONE first MCP slice:** authenticated API post/list routes exist for safe summaries; local stdio MCP and hosted JSON-RPC MCP expose `get_agent_stack_inventory`; generated docs advertise `11` hosted MCP tools; production `tools/list` and authenticated `tools/call` are live. **OPEN:** richer topology/drift query models and possible MCP resources.
