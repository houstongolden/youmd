...
13. Expose inventory topology through You.md API and MCP so authorized agents can inspect and repair drift. **DONE first MCP slice:** authenticated API post/list routes exist for safe summaries; local stdio MCP and hosted JSON-RPC MCP expose `get_agent_stack_inventory`; generated docs advertise `11` hosted MCP tools; production `tools/list` and authenticated `tools/call` are live; hosted MCP now reads the mirrored `agent-stack/` repo snapshot files. **OPEN:** richer topology/drift query models and possible MCP resources.
14. Build the web/Tauri Skill Mesh surface for diagram/report/ownership/DRY/catalog/machine drift views. **DONE first app slice / broader surface open:** Skills now has a `skill mesh` mode backed by Convex `agentStackInventories`, showing machine counts, ownership/sync/provenance rollups, catalog gaps, DRY queues, mirror samples, machine snapshots, and verify/compare commands. **OPEN:** richer diagram/HTML rendering, drilldowns, and Tauri-native packaging.
