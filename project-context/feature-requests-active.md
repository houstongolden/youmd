...
**Status:** IN PROGRESS (GitHub/portfolio graph hydration and bounded clean-root proof complete; production registry pending)
**Verified:** PARTIAL (focused CLI tests/build/dry-run, graph-backed clean-root clone/proof, and authenticated Machine pane visual proof passed; production registry still pending)
**Production Verified:** PARTIAL (production graph/proof APIs verified; hosted `/api/v1/skills` registry still pending)
...
9. Reseed/deploy the hosted skill registry so `machine-bootstrap` appears from `/api/v1/skills`, not only the local CLI catalog. **PENDING.**
