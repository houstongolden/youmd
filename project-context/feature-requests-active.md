...
**Status:** IN PROGRESS (production graph-backed planning, readiness audit, bounded package checks, and authenticated web/CLI proof complete; actual new-host run + local server smoke pending)
**Verified:** PARTIAL (`youmd machine prompt` / `youmd machine verify` unit tests/build, bounded package-check tests/build, root typecheck, docs generation/check, Convex codegen/deploy, production graph dry-run, compiled CLI prompt/readiness smokes, and authenticated local browser `/new computer` QA passed)
**Production Verified:** PARTIAL (Convex graph endpoint deployed; production dry-run fetched `55` projects / `40` tracked repos and selected `40` graph-backed cloneable repos; full new-host run pending)
...
3. Have the command install You.md, authenticate, pull/sync identity, restore shared skills/stacks/agent config, create `~/Desktop/CODE_YOU`, clone active GitHub-backed projects, restore encrypted env vaults, rehydrate local evidence, install daemons, and show status. **DONE in generated command; production graph-backed clone plan and post-clone readiness audit verified; pending real new-host execution and opt-in local server checks.**
...
7. Test on an actual brand-new computer / clean agent host. **PENDING:** run the generated command there, verify project clone count, shared skills/symlinks, local server startup for key projects, env vault restore, resident daemon status, and Portfolio Graph sync back to You.md.
8. Add a first-class post-clone readiness/run checker. **PARTIAL DONE:** `youmd machine verify` now reports git remotes, package managers/scripts, `.env.local` presence, `.env.example` presence, agent docs, and `project-context/` readiness without reading secret values, and the generated command runs it after clone/env restore. It now also supports opt-in bounded package checks with `--run-checks`, project caps, script selection, per-script timeout caps, and failure output tails. **PENDING:** clean-host dependency install and local server smoke/probe modes for selected key projects.
...
**Status:** IN PROGRESS (core implementation + real active-project hydration + local authenticated web proof + CLI/local-agent task/brain-dump proof + dashboard/API/MCP task triage + persisted shell update history/timeline complete for normal PR path + forced conflict-path regression proof; mobile/watch adapters, richer strategy enrichment, and repo-backed graph export still open)
**Verified:** PARTIAL (root build, Convex tests/typecheck, CLI build, local MCP smoke, shared-skill mirror sync, authenticated Chrome `/shell` repo-update QA, authenticated Codex in-app Browser `/shell` update QA, authenticated persisted update-history/timeline QA, forced 409 GitHub branch-recreation regression, authenticated persisted Portfolio Graph pane QA, authenticated 90-day GitHub project catalog refresh QA, authenticated real portfolio hydration from GitHub/local auditor data, authenticated shell-chat task/brain-dump repo-sync QA, authenticated CLI task/brain-dump repo-sync QA, built MCP registry proof, authenticated browser verification of CLI-created rows, authenticated dashboard task-triage QA, and API/local-agent task-triage proof passed)
**Production Verified:** PARTIAL (Convex backend deployed to `kindly-cassowary-600`; production web deploy still pending)
...
10. Expand project intelligence fields for founder focus: goals, vision, pain points, solution, positioning, audience, north star, metrics, constraints, not-building list, and competitors. **DONE first schema/API slice:** fields exist on `portfolioProjects`; content seeding/audit still pending.
...
**Status:** IN PROGRESS (local dashboard/data/CLI/shared-skill/MCP foundation shipped, persisted graph hydrated from real active projects, and locally visually verified; richer strategy/pattern enrichment + production verification pending)
**Verified:** PARTIAL (local TypeScript, CLI build, focused shared-skill source tests, portfolio audit smoke, env-key audit, shared skill mirror check, local stdio MCP smoke, production build, self-improvement CLI review, local skill install/sync, real active-project hydration through `portfolio-graph-auditor` + authenticated GitHub `trackedProjects`, and signed-in local visual QA passed)
**Production Verified:** NO
...
4. Add Project records that carry detailed and summarized AI descriptions, high-level goal, current focus, PRD/tasks/design/research/ideas/changelog/current-state/agent-doc paths, repo/local/docs/API/MCP URLs, stack, environment, and fresh-machine setup metadata. **DONE Convex first slice:** persisted `portfolioProjects` records exist and hydrate the dashboard; richer strategy content seeding remains pending.
...
11. Add a bundled YouStack skill or CLI command that audits local projects, prompts, docs, APIs, MCPs, stacks, code patterns, and UI patterns, then proposes graph/reuse updates. **DONE first slice:** shared `portfolio-graph-auditor`, bundled local CLI catalog visibility, `shared:` source install support, local `portfolio-graph-auditor` install/sync, and `youmd project portfolio-audit`; pending richer code/UI pattern mining.
...
**Status:** IN PROGRESS (local CLI/API/dashboard project-catalog slice complete; logged-in dashboard/prod verification pending)
**Verified:** PARTIAL (local server, CLI build/tests, GitHub 90-day dry-run, Convex codegen, TypeScript, and route smoke passed)
**Production Verified:** NO
...
7. Make secure `.env.local` transfer part of the fresh-machine flow through encrypted env backup/restore instructions and preflight checks; never print secrets. **DONE via canonical shared tooling:** machine-bootstrap now points to `env-key-audit.py`, `env-secure-backup.sh`, `env-backup-interactive.command`, and `env-secure-restore.sh`.
...
9. Add a dashboard/shell confirmation surface for tracked/synced computers, projects, skills/stacks, and env readiness. **PARTIAL:** dashboard project catalog cards now show repo/dir/stack/docs/progress metadata; machine/computer env-readiness confirmation still needs a dedicated signed-in surface.
...
**Status:** IN PROGRESS (GitHub 90-day hydration complete locally; production registry pending)
**Verified:** PARTIAL (focused CLI tests/build/dry-run passed; production registry/authenticated dashboard verification pending)
**Production Verified:** NO
