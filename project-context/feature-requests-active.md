...
**Status:** IN PROGRESS (core implementation + local authenticated proof complete; dashboard hydration, 90-day project catalog visibility, and brain-dump invocation proof still open)
**Verified:** PARTIAL (root build, Convex tests/typecheck, CLI build, local MCP smoke, shared-skill mirror sync, and authenticated Chrome `/shell` repo-update QA passed)
...
5. Add persisted Convex portfolio graph records instead of static-local data only. **DONE first schema/API slice:** `portfolioProjects`, `portfolioApiSurfaces`, `portfolioDependencyEdges`, `portfolioReusablePatterns`, `brainDumpCaptures`, and `portfolioTasks` tables plus owner-gated `portfolio.*` queries/mutations. **PENDING:** dashboard hydration and repo snapshot export.
...
7. Add `PROJECTS => Tasks` with personal/uncategorized tasks and project-associated tasks. **DONE first schema/API slice:** `portfolioTasks.projectSlug` is optional, so uncategorized/personal tasks and project tasks share one table.
