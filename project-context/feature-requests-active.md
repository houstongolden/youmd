...
15. Add resident realtime reconciliation so skills/stacks/context metadata stay synced across trusted machines and agents. **DONE first inventory slice + status surface / broader repair loop open:** `youmd sync --live --daemon` now runs bounded `youmd skill inventory --out-dir ~/.youmd/agent-stack-inventory --sync` every 30 minutes by default, with env knobs for disabling/tuning/output directory. `youmd skill inventory status` now lists the latest local proof, synced machine inventory summaries, count drift, and exact refresh/verify/diff commands for Mac mini comparison. **OPEN:** richer drift repair actions and second-Mac proof.
