...
15. Add resident realtime reconciliation so skills/stacks/context metadata stay synced across trusted machines and agents. **DONE first inventory slice / broader repair loop open:** `youmd sync --live --daemon` now runs bounded `youmd skill inventory --out-dir ~/.youmd/agent-stack-inventory --sync` every 30 minutes by default, with env knobs for disabling/tuning/output directory. **OPEN:** richer drift repair actions and second-Mac proof.
