...
9. Prepare the CLI publish target so the Mac mini can install the new commands through normal `youmd` update/install. **DONE locally:** CLI package is bumped to `0.8.11` for the inventory commands plus resident Skill Mesh refresh. **PENDING:** build/pack verification, npm publish/OTP, and Mac mini install proof.
