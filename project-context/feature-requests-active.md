...
**Status:** IN PROGRESS (artifact workspace + Loop Reports foundation + first private DSI weather/surf adapter + public profile conversation API/MCP first slice code complete locally)
...
8. Add DSI catalog components that can render privately in `/shell` and optionally on public profiles with owner approval. **PARTIAL CODE COMPLETE:** private `dsiComponents` substrate, Reports DSI Catalog panel, refresh control, and markdown artifacts under `dsi/private/*` are implemented for weather/surf. **PENDING:** custom component editor/schema and owner-approved public profile rendering.
