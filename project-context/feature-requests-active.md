...
**Status:** INSTALL-TIME LOCAL INVENTORY WIRED / CLI 0.8.10 PUBLISH TARGET READY / CONVEX+API+CLI SYNC PERSISTENCE DONE / GITHUB SNAPSHOT PROVEN / MCP EXPOSED / APP+REALTIME+SECOND-MAC STILL OPEN
...
10. Productize install-time inventory in `install.sh` / machine setup so a new Mac automatically inventories and verifies the skill/stack system after install. **DONE local report + Convex/GitHub/MCP sync slice:** CLI package bundles the scanner, curl install writes best-effort `~/.youmd/agent-stack-inventory`, fresh-machine bootstrap runs inventory after shared skills/MCP restore and near final proof, and both paths call `youmd skill inventory --sync` so authenticated installs persist a safe summary and attempt the repo snapshot path. The scanner is now bounded with `walkIssues` metadata so install-time scans do not hang indefinitely on huge local trees. **OPEN:** npm publish + second-machine proof.
