...
5. Clarify npm status. **UPDATED:** local CLI is now `0.8.5`, npm latest is still behind, and the fresh-machine prompt/ystack/realtime-daemon/Secret-Vault-status changes require publishing `0.8.5` for npm fallback and `npx youmd@latest`. Curl install defaults to GitHub source/main once this commit is pushed.
...
5. Publish/install everywhere. **PENDING:** publish CLI `0.8.5` to npm and rerun the Mac mini setup so the new machine has the realtime daemon and account Secret Vault status from the normal install path.
