...
14. Build the web/Tauri Skill Mesh surface for diagram/report/ownership/DRY/catalog/machine drift views. **DONE first app + drift slice / broader surface open:** Skills now has a `skill mesh` mode backed by Convex `agentStackInventories` plus `getAgentStackInventoryDrift`, showing machine counts, ownership/sync/provenance rollups, catalog gaps, DRY queues, mirror samples, machine snapshots, drift baseline/summary, per-machine status labels, and verify/compare/refresh commands. **OPEN:** richer diagram/HTML rendering, drilldowns, and Tauri-native packaging.
