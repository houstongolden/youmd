...
**Status:** IN PROGRESS (core implementation + local authenticated proof complete; 90-day project catalog visibility, task/brain-dump UI invocation, and update-artifact history still open)
**Verified:** PARTIAL (root build, Convex tests/typecheck, CLI build, local MCP smoke, shared-skill mirror sync, authenticated Chrome `/shell` repo-update QA, and authenticated persisted Portfolio Graph pane QA passed)
...
7. Add `PROJECTS => Tasks` with personal/uncategorized tasks and project-associated tasks. **DONE first schema/API slice:** `portfolioTasks.projectSlug` is optional, so uncategorized/personal tasks and project tasks share one table. **PENDING:** first dashboard/agent task creation and editing workflow.
8. Add task owner separation: Houston/human versus agents. **DONE first schema/API slice:** `portfolioTasks.ownerType` is `human` or `agent` with optional `ownerLabel`.
9. Add a cross-project synced brain-dump-to-tasks skill/workflow. **DONE shared-skill first slice:** canonical shared `braindump-task-router` preserves raw dumps, summarizes insights, routes to projects, separates human/agent tasks, and points writes at You.md `brainDumpCaptures` + `portfolioTasks`. Shared mirrors are synced into Claude/Codex/Cursor/Pi. **PENDING:** first web/CLI invocation proof through the You.md app/CLI.
...
11. Analyze recent GitHub activity from the last 90 days and make it visible in the project intelligence layer. **PARTIAL:** existing `analyzeActiveProjects` persists `trackedProjects` and repo-backed `projects/*.md`; needs a fresh authenticated run and dashboard proof in this continuation.
12. Verify via authenticated local CLI shell + authenticated Chrome web testing, not smoke tests only. **PARTIAL DONE:** `get_project_context` local MCP smoke proved project-scoped `portfolioGraph` for `youmd`; authenticated Chrome clicked `[ update ]` and observed `SYNCING` -> `SYNCED / REPO MIRROR CURRENT / JUST NOW` with the merged PR transcript; authenticated Chrome verified the Portfolio Graph pane loading persisted Convex records and refreshing the seed graph. **PENDING:** web-shell chat-initiated content-change proof and 90-day project catalog dashboard proof.
...
3. Identify GitHub repositories active in the last 30-90 days from authenticated GitHub data. **DONE locally:** `gh api /user/repos` found 41 repos pushed in the last 90 days.
4. Compare recent repos against You.md tracked projects and add/refresh missing project records. **DONE locally for CLI planner + Convex action path; production dashboard refresh pending.**
