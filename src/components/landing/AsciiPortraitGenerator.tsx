...
function imgToAscii(
  ...
  const rows = Math.floor(
    cols * (imgEl.naturalHeight / imgEl.naturalWidth) * 0.46
  );
  c.width = cols;
  c.height = rows;
...
const AsciiPortraitGenerator = () => {
  ...
  const [asciiData, setAsciiData] = useState<AsciiCell[][] | null>(
    null
  );
  ...
  useEffect(() => {
    ...
    const t = setInterval(
      () => setLoadFrame((f) => (f + 1) % LOAD_FRAMES.length),
      120
    );
  ...
  useEffect(() => {
    if (asciiData && portraitRef.current)
      renderPortrait(portraitRef.current, asciiData);
  }, [asciiData]);
        ...
        {!asciiData && (
          <div className="text-center p-8">
            {imgLoaded ? (
              ...
              <>
                <p className="text-muted-foreground/40 font-mono text-2xl mb-3">
                  [ ]
                </p>
                ...
                <>
                  ...
                  <button
                    onClick={() => {
                      ...
                      link.href =
                        portraitRef.current.toDataURL("image/png");
                      link.click();
                    }}
                    className="cta-outline px-3 py-2.5 font-mono text-[10px]"
                  >
      ...
      {tab === "about" && (
        <div className="terminal-panel rounded-tl-none p-5 font-mono text-[10px] leading-[1.9] space-y-4">
          ...
          <p className="text-muted-foreground">
            YOU is an identity context protocol -- like MCP, but for
            your human identity.
            <br />
            Give any AI agent structured context about who you are,
            how you work,
            <br />
            and what you care about. One file. Every agent. Every
            session.
          </p>
          ...
          <p className="text-muted-foreground">
            ...
            <br />
            3. pixel&rarr;ascii algorithm maps brightness to chars
            <br />
          ...
          <p className="text-muted-foreground/50">
            stack: convex &middot; openrouter &middot; perplexity
            &middot; apify &middot; firecrawl
            <br />
            byok: ~/.you/config.json &middot; never transits backend
          </p>
        </div>
      )}
    </div>
