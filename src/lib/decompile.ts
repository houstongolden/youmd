...
export function decompileBundle(youJson: any, youMd: string): VirtualFile[] {
  ...
  // MINIMAL scaffold — only the core files every user should have.
  // We do NOT scaffold:
  // - voice.linkedin.md/voice.x.md/etc (only show platform variants if real data exists)
  // - projects/README.md (only show projects/ dir if it has real subdirs)
  // - sessions/history.md, memory/index.md (generated by generateMemoryFiles)
  // - sources/README.md, skills/README.md (these are CLI-managed, not user-edited)
  // - private/notes.md etc as scaffold (real ones come from privateContext)
  //
  // Result: a clean tree where empty dirs disappear and the user only sees
  // what actually has content or is essential to fill in.
  const scaffold: Array<{ path: string; content: string; section: string; editable: boolean }> = [
    // Profile (public — every user needs these)
