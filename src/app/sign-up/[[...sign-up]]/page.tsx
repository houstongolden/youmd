...
export default function SignUpPage() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      // Clerk v7: use signUp.password() which accepts email + password + username
      ...
      if (pwError) {
        setError(pwError.message ?? "initialization failed.");
        setLoading(false);
        return;
      }

      // Check if sign-up is complete or needs verification
      if (signUp.status === "complete") {
        const { error: finalizeError } = await signUp.finalize({
          navigate: () => router.push("/claim"),
        });
        if (finalizeError) {
  ...
  const handleVerify = async (e: FormEvent) => {
    ...
    try {
      ...
      if (verifyErr) {
        setVerifyError(verifyErr.message ?? "invalid code. try again.");
        setVerifying(false);
        return;
      }
      ...
      if (signUp.status === "complete") {
        ...
        if (finalizeError) {
          setVerifyError(
            finalizeError.message ?? "failed to finalize session."
          );
        }
      } else {
        setVerifyError("verification incomplete. check your code.");
      }
    } catch (err: unknown) {
              ...
              <div className="p-6">
                {!showVerification ? (
                  ...
                  <>
                    ...
                    <p className="font-mono text-[11px] text-[hsl(var(--text-secondary))] opacity-60 mb-6 leading-relaxed">
                    ...
                    <div className="mt-4">
                      <button
                        ...
                        className="font-mono text-[11px] text-[hsl(var(--text-secondary))] opacity-50 hover:text-[hsl(var(--accent))] transition-colors"
                      >
                        &lt; back
                      </button>
                    </div>
