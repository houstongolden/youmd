...
export type RightPane = "home" | "tasks" | "profile" | "portfolio" | "portrait" | "edit" | "files" | "share" | "skills" | "stacks" | "history" | "settings" | "analytics" | "agents" | "live-agents" | "vault" | "help" | "github" | "apis" | "machine";
...
export function buildPublicShareBlock(
  ...
  return [
    ...
    ``,
    `If the URL above fails, try these direct endpoints:`,
    `- JSON: ${origin}/${_username}/you.json`,
    `- Markdown: ${origin}/${_username}/you.md`,
    `- Plain text: ${origin}/${_username}/you.txt`,
    ``,
    `IMPORTANT — be honest about what you can and can't do:`,
    `- If you DO have web browsing/fetch capability: fetch the URL now.`,
    `- If you DO NOT have a browsing/fetch tool enabled in this session,`,
    `  say so explicitly. Do NOT claim it is "blocked by robots.txt" or`,
    `  "blocked for security" or any other excuse. The URL is public and`,
    `  serves AI agents by design. The honest answer is "I don't have a`,
    `  web browsing tool enabled here — please enable it or paste the`,
    `  content directly."`,
    ``,
...
export function buildProjectShareBlock(
  ...
  return [
    ...
    ``,
    `IMPORTANT: if you don't have a web browsing/fetch tool enabled in this`,
    `session, say so explicitly. Do NOT claim it's blocked by robots.txt or`,
    `security policy. The URL is designed to be fetched by AI agents.`,
    ``,
