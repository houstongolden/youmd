...
bash scripts/env-vault/backup.sh
```
...
bash restore.sh /path/to/env-vault-<date>.tar.<ext>

# If project dirs already exist with .env.local files:
bash restore.sh --force /path/to/env-vault-<date>.tar.<ext>
```
...
These scripts are designed to graduate into first-class `youmd` CLI commands:

```bash
youmd env backup    # → calls backup.sh logic
youmd env restore   # → calls restore.sh logic
youmd env status    # → lists projects and their .env.local state
```

When implementing, use `youmd`'s existing BrailleSpinner for async operations
and the standard security model (no values in logs or terminal output).
