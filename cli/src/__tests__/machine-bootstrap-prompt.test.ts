...
describe("fresh machine bootstrap prompt", () => {
  it("builds one pasteable shell command with portfolio graph hydration and project clone steps", () => {
    ...
    expect(command).toContain("account-backed Secret Vault restore not available yet; falling back to local/iCloud vault discovery");
    expect(command).toContain("run this on the source Mac: youmd env vault share");
    ...
    expect(command).toContain('strict proof requires Secret Vault restore or YOUMD_ENV_VAULT');
    expect(command).toContain('[ "${YOUMD_REQUIRE_ENV_VAULT:-}" = "1" ]');
