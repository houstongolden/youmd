...
describe("fresh machine bootstrap prompt", () => {
  it("builds one pasteable shell command with portfolio graph hydration and project clone steps", () => {
    ...
    expect(command).toContain('run_with_timeout "$HYDRATE_TIMEOUT" you project portfolio-hydrate');
    expect(command).toContain('you login --key "$YOU_BOOTSTRAP_API_KEY"');
    expect(command).toContain("you mcp --install claude --auto || true");
    expect(command).toContain("you mcp --install codex --auto || true");
    expect(command).toContain("run_agent_stack_inventory");
    ...
    expect(command).toContain('you project portfolio-hydrate --root "$ROOT" --days "$DAYS" --limit "$LIMIT"');
    ...
    expect(command).toContain('if you machine --help 2>/dev/null | grep -q -- "--recent-only"; then');
    expect(command).toContain("RECENT_ONLY_ARGS=(--recent-only)");
    expect(command).toContain("run_machine_projects_recent_only()");
    expect(command).toContain('you machine projects "$@" </dev/null');
    expect(command).toContain('PROJECT_ARGS=(--root "$ROOT" --days "$DAYS" "${RECENT_ONLY_ARGS[@]}")');
    expect(command).toContain('MAX_CLONE_PROJECTS="${YOU_MAX_CLONE_PROJECTS:-${YOUMD_MAX_CLONE_PROJECTS:-}}"');
    expect(command).toContain('PROJECT_ARGS+=(--max-clone-projects "$MAX_CLONE_PROJECTS")');
    ...
    expect(command).toContain('YOU_EXPAND_TO_90_DAYS');
    ...
    expect(command).toContain('you env backup --root "$ROOT" --preflight');
    expect(command).toContain('checking You.md Secret Vault for the latest encrypted env vault');
    expect(command).toContain('you env vault device-register');
    expect(command).toContain('checking You.md Secret Vault for the latest encrypted env vault and trusted-device envelope');
    expect(command).toContain('you env vault pull --out "$SECRET_VAULT_DIR" --restore --root "$ROOT" --map-existing --existing-only --skip-agent-auth');
    expect(command).toContain("restored env vault through trusted-device Secret Vault");
    expect(command).toContain('ENV_VAULT_PATH="${YOU_ENV_VAULT:-${YOUMD_ENV_VAULT:-}}"');
    expect(command).toContain('ALLOW_LOCAL_ENV_VAULT_FALLBACK="${YOU_ALLOW_LOCAL_ENV_VAULT_FALLBACK:-${YOUMD_ALLOW_LOCAL_ENV_VAULT_FALLBACK:-0}}"');
    expect(command).toContain("account-backed Secret Vault restore is waiting for a trusted-device share");
    expect(command).toContain("source Mac action: you env vault share");
    ...
    expect(command).toContain("YOU_ALLOW_LOCAL_ENV_VAULT_FALLBACK=1 or YOU_ENV_VAULT=/path/to/env-vault");
    ...
    expect(command).toContain('export YOU_ENV_VAULT="$ENV_VAULT_PATH"');
    ...
    expect(command).toContain('if [ ! -f "$ENV_VAULT_PATH" ]; then');
    expect(command).toContain('YOU_ENV_VAULT_KEYCHAIN_SERVICE:-${YOUMD_ENV_VAULT_KEYCHAIN_SERVICE:-you-env-vault}');
    ...
    expect(command).toContain('you env restore "$ENV_VAULT_PATH" --root "$ROOT" --list --map-existing --existing-only --skip-agent-auth');
    expect(command).toContain('you env restore "$ENV_VAULT_PATH" --root "$ROOT" --map-existing --existing-only --skip-agent-auth');
    ...
    expect(command).toContain("you env vault push --root ~/Desktop/CODE_2025 --out ~/Desktop/youmd-env-vault");
    expect(command).toContain("you env vault share");
    expect(command).toContain("you env backup --root ~/Desktop/CODE_2025 --out ~/Desktop/youmd-env-vault");
    ...
    expect(command).toContain("YOU_ALLOW_LOCAL_ENV_VAULT_FALLBACK=1 YOU_REQUIRE_ENV_VAULT=1 <same command>");
    expect(command).toContain("YOU_ENV_VAULT=/path/to/env-vault-YYYYMMDDTHHMMZ.tar.enc YOU_REQUIRE_ENV_VAULT=1 <same command>");
    ...
    expect(command).toContain('[ "${YOU_REQUIRE_ENV_VAULT:-${YOUMD_REQUIRE_ENV_VAULT:-}}" = "1" ]');
    expect(command).toContain('you machine verify --root "$ROOT" --max-projects "$LIMIT" --write-report --sync-report');
    expect(command).toContain("running final one-command machine sync/proof consolidation");
    expect(command).toContain('you machine sync-now --root "$ROOT" --max-projects "$LIMIT"');
    expect(command).toContain('YOU_RUN_CHECKS');
    expect(command).toContain('--run-checks --max-check-projects "${YOU_MAX_CHECK_PROJECTS:-${YOUMD_MAX_CHECK_PROJECTS:-8}}"');
  ...
  it("renders a copyable agent prompt with secret-safe env-vault language", () => {
    ...
    expect(prompt).toContain("you skill inventory");
    expect(prompt).toContain("you machine sync-now");
    expect(prompt).toContain("you env vault pull");
    expect(prompt).toContain("you env vault device-register");
    ...
    expect(prompt).toContain("stop with the exact source-Mac `you env vault share` action");
    expect(prompt).toContain("Local/iCloud passphrase fallback runs only when `YOU_ALLOW_LOCAL_ENV_VAULT_FALLBACK=1`");
    expect(prompt).toContain("reconcile `you pull && you sync` after env-vault handling");
    expect(prompt).toContain("do not leave this Mac showing `remote ahead`");
    expect(prompt).toContain("local vault passphrase/Keychain only when `YOU_ALLOW_LOCAL_ENV_VAULT_FALLBACK=1`");
    expect(prompt).toContain("after the new Mac registers itself run `you env vault share`");
    expect(prompt).toContain("per-device encrypted passphrase envelopes");
    expect(prompt).toContain("YOU_MAX_CLONE_PROJECTS");
    expect(prompt).toContain("YOU_REQUIRE_ENV_VAULT=1");
    ...
    expect(prompt).toContain("you env vault push --root ~/Desktop/CODE_2025 --out ~/Desktop/youmd-env-vault");
    expect(prompt).toContain("you env vault share");
    expect(prompt).toContain("you env backup --root ~/Desktop/CODE_2025 --out ~/Desktop/youmd-env-vault");
    ...
    expect(prompt).toContain("YOU_ALLOW_LOCAL_ENV_VAULT_FALLBACK=1");
    expect(prompt).toContain("YOU_ENV_VAULT=/path/to/env-vault-*.tar.enc");
