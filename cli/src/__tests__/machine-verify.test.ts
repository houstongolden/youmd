...
describe("machine readiness verifier", () => {
  ...
  it("loads the latest agent stack inventory into machine proof", () => {
    ...
    expect(proof.summary.warnings).not.toContain("agent stack inventory proof missing; run youmd skill inventory --out-dir ~/.youmd/agent-stack-inventory --register-catalog --sync");
  });
  ...
  it("warns when machine proof explicitly has no agent stack inventory", () => {
    ...
    expect(proof.summary.warnings).toContain("agent stack inventory proof missing; run youmd skill inventory --out-dir ~/.youmd/agent-stack-inventory --register-catalog --sync");
  });
