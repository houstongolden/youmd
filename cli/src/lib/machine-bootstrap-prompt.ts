...
export function buildFreshMachineBootstrapPrompt(options: FreshMachineBootstrapOptions = {}): string {
  ...
  return [
    ...
    "```",
    "",
    `What it does: installs You.md, authenticates, pulls/syncs identity, restores shared agent skills/stacks, hydrates the portfolio graph, creates ${root}, clones active projects from the last ${days} days, restores an encrypted env vault if supplied, rehydrates local evidence, and starts resident sync daemons.`,
    "",
