...
export function buildFreshMachineBootstrapScript(): string {
  return [
    ...
    'youmd machine verify --root "$ROOT" --max-projects "$LIMIT" --write-report || true',
    'if [ "${YOUMD_RUN_CHECKS:-}" = "1" ]; then',
    '  echo "[you.md] running bounded package checks"',
    '  youmd machine verify --root "$ROOT" --max-projects "$LIMIT" --run-checks --max-check-projects "${YOUMD_MAX_CHECK_PROJECTS:-8}" --check-timeout-ms "${YOUMD_CHECK_TIMEOUT_MS:-120000}" --write-report || true',
    "else",
    ...
    "  fi",
    '  youmd machine verify "${VERIFY_ARGS[@]}" --write-report || true',
    "else",
